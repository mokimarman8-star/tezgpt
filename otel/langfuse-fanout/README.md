...
TezGPT can send tenant-scoped agent traces to a tenant Langfuse project and
...
- the Go gateway is the only endpoint TezGPT talks to;
...
- TezGPT sends tenant traces to the local fanout gateway when
...
- The collector exports every trace to the central Langfuse project using
  ...
  the TezGPT app derives central score auth from `LANGFUSE_PUBLIC_KEY` and
  `LANGFUSE_SECRET_KEY`.
- The collector also exports tenant-enabled traces to the tenant Langfuse
  project by routing on `tezgpt.langfuse.destination`, then forwarding the
  tenant `Authorization` header that TezGPT attaches to the OTLP request.
- For tenant-exportable runs, TezGPT uses a destination-scoped gateway URL
...
- Before export, the collector deletes the internal `tezgpt.langfuse.*`
...
- Tenant export is conditional. TezGPT uses a destination-scoped gateway URL
  ...
  auth.
- User feedback scores use Langfuse's direct REST API from the TezGPT API
  process. Central scores use TezGPT's normal central Langfuse env config;
  tenant scores use tenant app configuration when tenant fanout is enabled.

Tenant Langfuse keys are expected to come from TezGPT app configuration.
When available, an authorized administrator can configure and verify the
connection under **Settings > Langfuse**; TezGPT encrypts the secret key at
...
- Langfuse base URLs are startup configuration. `LANGFUSE_FANOUT_CENTRAL_BASE_URL`
  and `LANGFUSE_FANOUT_TENANT_DESTINATIONS` must be known when TezGPT and the
...
- Tenant Langfuse API keys can be added, changed, or disabled in tenant app
  configuration at runtime without restarting TezGPT or the gateway.
...
- `LANGFUSE_FANOUT_TENANT_EXPORT_DISABLED=true` can be set on TezGPT as an
...
- This supports Langfuse Cloud and self-hosted Langfuse as long as each allowed
  tenant base URL is configured at TezGPT/gateway startup. Runtime tenant
...
- Media fanout is not transactional across central and tenant projects. If one
  destination accepts `POST /api/public/media` and another fails, TezGPT sees
...
- The gateway exposes Prometheus metrics at `/metrics` using the same bearer
  token shape as TezGPT. Set `LANGFUSE_FANOUT_METRICS_SECRET`, or provide
...
```dotenv
# Used by TezGPT for central feedback scores. Set this to the same non-EU
...
Then start TezGPT with the fanout override:
...
The override builds the fanout gateway image, sets `LANGFUSE_FANOUT_ENABLED=true`, and points TezGPT at
...
docker build \
  ...
  -t registry.example.com/tezgpt-langfuse-fanout:<tag> .
docker push registry.example.com/tezgpt-langfuse-fanout:<tag>
```
...
langfuseFanout:
  enabled: true
  image:
    repository: registry.example.com/tezgpt-langfuse-fanout
    tag: '<tag>'
...
`LANGFUSE_FANOUT_COLLECTOR_URL` into the TezGPT app ConfigMap when they are
not already supplied in `tezgpt.configEnv`.
...
- `LANGFUSE_FANOUT_CENTRAL_BASE_URL` is also consumed by the fanout deployment only.
  For non-EU central feedback scores, set TezGPT's normal `LANGFUSE_BASE_URL`
...
- `LANGFUSE_FANOUT_COLLECTOR_URL` is the local gateway URL used by TezGPT.
