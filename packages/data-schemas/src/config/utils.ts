 ...
 * Priority:
 * 1. TEZGPT_LOG_DIR environment variable
 * 2. If running within TezGPT monorepo (when cwd ends with /api), use api/logs
...
export const getLogDirectory = (): string => {
  if (process.env.TEZGPT_LOG_DIR) {
    return process.env.TEZGPT_LOG_DIR;
  }
  ...
  // For TezGPT project structure, use api/logs
  // For external consumers, they should set TEZGPT_LOG_DIR
  if (cwd.includes('TezGPT')) {
    return apiLogsPath;
  }
