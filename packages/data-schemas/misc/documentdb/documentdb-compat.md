...
- **Transactions**: supported on 4.0+ instance-based ("Amazon DocumentDB …
  ...
  unsupported on 3.6 and elastic. TezGPT already probes at runtime
  ...
  in transactions, no retryable commit/abort) don't intersect TezGPT's
  usage.
...
- **TTL indexes**: supported everywhere including elastic. AWS warns deletion
  ...
  specific period") — acceptable, since TezGPT treats TTL as cleanup, not
...
| Capability (TezGPT dependency) | 3.6 | 4.0 | 5.0 | 8.0 | Elastic |
| --- | --- | --- | --- | --- | --- |
