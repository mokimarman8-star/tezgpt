 ...
 *
 * Mirrors a minimal subset of `isPrivateIP` from `@tezgpt/api` to avoid a
...
/**
 * Mirrors `hasPrivateEmbeddedIPv4` in `@tezgpt/api`'s ip helpers: 6to4, NAT64, and Teredo
...
/**
 * Mirrors the allowedAddresses parser in `@tezgpt/api`'s auth helpers.
 * Kept as a local copy because the data-provider package cannot import from
 * `@tezgpt/api` without creating a circular dependency. Keep the two
...
export const baseEndpointSchema = z.object({
   ...
   * (`${VAR}`), user fields (`{{TEZGPT_USER_*}}`), and request-body fields
   * (`{{TEZGPT_BODY_CONVERSATIONID}}`). Primarily for routing built-in
 ...
 * - `enabled: false` is a TezGPT-only kill switch that disables the entire
 ...
 * `(options?) => ToolApprovalHookFactory` (see `@tezgpt/api`'s `registerToolApprovalHook`).
 ...
 * block, TezGPT persists checkpoints to its primary MongoDB, so resume works
...
export const agentsEndpointSchema = baseEndpointSchema
  ...
  .merge(
    z.object({
       ...
       * disables the guard for that tool only. Merged over TezGPT's shipped default of
       * `{ create_file: 131072 }`. */
...
export enum SettingsTabValues {
  ...
  /**
   * Tab for TezGPT BYOK (Bring Your Own Key) settings
   */
  BYOK = 'byok',
  /**
...
export enum Constants {
  /**
   * Key for the app's version. The placeholder `__TEZGPT_VERSION__` is
   ...
   */
  VERSION = '__TEZGPT_VERSION__',
  /** Key for the Custom Config's version (tezgpt.yaml). */
