...
/** Betas TezGPT injects itself, safe to strip from persisted AMRF when a
 ...
 * three user-facing modes — `'auto'` (TezGPT decides), `'summarized'`, and
 ...
 * used when the TezGPT model ID maps to an application inference profile).
...
export const bedrockInputSchema = s.tConversationSchema
  .pick({
    /* TezGPT params; optionType: 'conversation' */
    chatProjectId: true,
    modelLabel: true,
    promptPrefix: true,
    resendFiles: true,
  ...
  .transform((data) => {
    ...
    if (
      ...
      if (!isAnthropicModel) {
         ...
         * TezGPT-generated betas (output-128k, fine-grained tool streaming);
