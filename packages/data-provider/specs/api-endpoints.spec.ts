...
describe('buildLoginRedirectUrl', () => {
  ...
  it('returns plain /login for basename-prefixed /login (e.g. /tezgpt/login)', () => {
    window.history.replaceState({}, '', '/tezgpt/login?redirect_to=%2Fc%2Fabc');
    const result = buildLoginRedirectUrl();
    expect(result).toBe('/login');
  });

  it('returns plain /login for basename-prefixed /login sub-paths', () => {
    const result = buildLoginRedirectUrl('/tezgpt/login/2fa', '', '');
    expect(result).toBe('/login');
  });
