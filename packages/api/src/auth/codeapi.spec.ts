...
const mockGetTenantId = jest.requireMock('@tezgpt/data-schemas').getTenantId as jest.Mock;
...
describe('Code API JWT minting', () => {
  ...
  beforeEach(() => {
    ...
    process.env.CODEAPI_AUTH_PROVIDER = 'tezgpt-jwt';
    ...
    process.env.CODEAPI_JWT_ISSUER = 'tezgpt';
  ...
  it('mints a Code API-scoped token from canonical TezGPT JWT context', async () => {
  ...
  it('marks OpenID users authenticated by TezGPT JWT as TezGPT JWT callers', async () => {
    process.env.OPENID_REUSE_TOKENS = 'true';

    const token = await mintCodeApiToken(baseRequest({ provider: 'openid' }));
    const { claims } = decodeToken(token);

    expect(claims.principal_source).toBe('tezgpt_jwt');
  });
