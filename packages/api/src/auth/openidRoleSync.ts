...
import { SystemRoles } from 'tezgpt-data-provider';
...
type TezGPTRolesForOpenIdSyncInput = {
  rolePriority: string[];
  fallbackRole?: string;
...
/**
 * Gets the configured TezGPT roles that OpenID role sync is allowed to assign.
 * The names are read from config, checked against storage, and returned as canonical role names.
 */
export async function getTezGPTRolesForOpenIdSync(
  input: TezGPTRolesForOpenIdSyncInput,
): Promise<TezGPTRolesForOpenIdSync> {
...
/**
 * Chooses the TezGPT role to assign from normalized OpenID token values and validated config.
...
export function selectOpenIdRole(
  ...
  // Keep only OpenID roles that can map to configured TezGPT roles.
