...
import { EXTENSION_HOOKS_FILE, TEZGPT_EXTENSION_NAMESPACE } from './constants';
...
export async function reportUnexecutedHooks(realRoot: string): Promise<PluginDiagnostic[]> {
  const location = `${TEZGPT_EXTENSION_NAMESPACE}/${EXTENSION_HOOKS_FILE}`;
...
/**
 * Hooks are outside the portable Agent Plugins v1 format, so TezGPT reads
