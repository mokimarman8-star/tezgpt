import { extractEnvVariable } from 'tezgpt-data-provider';
 ...
 *
 * NOTE: This is a TezGPT-specific encoding scheme to work around Fetch API limitations.
...
export function createSafeUser(
  ...
  // user objects), so `{{TEZGPT_USER_ID}}` placeholders still resolve.
 ...
 * `{{TEZGPT_USER_*}}`, `{{TEZGPT_BODY_*}}`, and `{{TEZGPT_OPENID_*}}`
 * names. Deliberately excludes unknown names (a typo'd placeholder staying
 * literal is diagnosable) and `{{TEZGPT_GRAPH_ACCESS_TOKEN}}`, which is
 * resolved asynchronously via the OBO flow outside this pipeline.
 */
const RESOLVABLE_PLACEHOLDER_PATTERN = new RegExp(
  [
    `TEZGPT_USER_(?:${ALLOWED_USER_FIELDS.map((field) => field.toUpperCase()).join('|')})`,
    `TEZGPT_BODY_(?:${ALLOWED_BODY_FIELDS.map((field) => field.toUpperCase()).join('|')})`,
    `TEZGPT_OPENID_(?:${OPENID_TOKEN_FIELDS.join('|')}|TOKEN)`,
  ]
    .map((names) => `\\{\\{(?:${names})\\}\\}`)
    .join('|'),
  'g',
 ...
 * TezGPT's internal template syntax is never sent upstream as if it were
 * real user data (e.g. a gateway trusting a literal
 * `{{TEZGPT_USER_OPENIDID}}` as an account identity would pool unrelated
...
function processUserPlaceholders(
  ...
  for (const field of ALLOWED_USER_FIELDS) {
    const placeholder = `{{TEZGPT_USER_${field.toUpperCase()}}}`;
 ...
 * Recognized placeholders: `{{TEZGPT_BODY_<FIELD>}}` where `<FIELD>` ∈ ALLOWED_BODY_FIELDS.
...
function processBodyPlaceholders(value: string, body: RequestBody): string {
  ...
  for (const field of ALLOWED_BODY_FIELDS) {
    const placeholder = `{{TEZGPT_BODY_${field.toUpperCase()}}}`;
