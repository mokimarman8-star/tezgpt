...
import type { IUser, OIDCTokens } from '@tezgpt/data-schemas';
 ...
 */
export const GRAPH_TOKEN_PLACEHOLDER = '{{TEZGPT_GRAPH_ACCESS_TOKEN}}';
...
export function processOpenIDPlaceholders(
  ...
  for (const field of OPENID_TOKEN_FIELDS) {
    const placeholder = `{{TEZGPT_OPENID_${field}}}`;
  ...
  const genericPlaceholder = '{{TEZGPT_OPENID_TOKEN}}';
