...
describe('getBasePath', () => {
  ...
  it('should return base path for subdirectory deployment', () => {
    ...
    expect(getBasePath()).toBe('/tezgpt');
  });
  ...
  it('should handle nested subdirectories', () => {
    ...
    expect(getBasePath()).toBe('/apps/tezgpt');
  });
  ...
  it('should handle URLs with query parameters', () => {
    process.env.DOMAIN_CLIENT = 'http://localhost:3080/tezgpt?param=value';
    expect(getBasePath()).toBe('/tezgpt');
  });

  it('should handle URLs with fragments', () => {
    process.env.DOMAIN_CLIENT = 'http://localhost:3080/tezgpt#section';
    expect(getBasePath()).toBe('/tezgpt');
  });
  ...
  it('should handle URLs with ports', () => {
    process.env.DOMAIN_CLIENT = 'http://localhost:8080/tezgpt';
    expect(getBasePath()).toBe('/tezgpt');
  });

  it('should handle URLs with subdomains', () => {
    process.env.DOMAIN_CLIENT = 'https://app.example.com/tezgpt';
    expect(getBasePath()).toBe('/tezgpt');
  });
});
