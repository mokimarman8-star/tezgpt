...
import { TokenExchangeMethodEnum } from 'tezgpt-data-provider';
import type { MCPOptions } from 'tezgpt-data-provider';
import type { IUser } from '@tezgpt/data-schemas';
import {
  resolveNestedObject,
  encodeHeaderValue,
  resolveHeaders,
...
describe('resolveHeaders', () => {
  ...
  it('should process full user object placeholders', () => {
    ...
    const headers = {
      'User-Email': '{{TEZGPT_USER_EMAIL}}',
      'User-Name': '{{TEZGPT_USER_NAME}}',
      'User-Username': '{{TEZGPT_USER_USERNAME}}',
      'User-Role': '{{TEZGPT_USER_ROLE}}',
      'User-Id': '{{TEZGPT_USER_ID}}',
      'Content-Type': 'application/json',
    };
  ...
  it('should handle missing user fields gracefully', () => {
    ...
    const headers = {
      'User-Email': '{{TEZGPT_USER_EMAIL}}',
      'User-Username': '{{TEZGPT_USER_USERNAME}}',
      'Non-Existent': '{{TEZGPT_USER_NONEXISTENT}}',
    };
  ...
  it('should prioritize custom user variables over user fields', () => {
    ...
    const customUserVars = {
      TEZGPT_USER_EMAIL: 'custom-email@example.com',
    };

    const headers = {
      'Test-Email': '{{TEZGPT_USER_EMAIL}}',
    };
  ...
  it('should handle multiple occurrences of the same placeholder', () => {
    ...
    const headers = {
      'Primary-Email': '{{TEZGPT_USER_EMAIL}}',
      'Secondary-Email': '{{TEZGPT_USER_EMAIL}}',
      'Backup-Email': '{{TEZGPT_USER_EMAIL}}',
    };
  ...
  it('should handle mixed variable types in the same headers object', () => {
    ...
    const headers = {
      ...
      'X-User-Id': '{{TEZGPT_USER_ID}}',
      'X-System-Key': '${TEST_API_KEY}',
      'X-User-Email': '{{TEZGPT_USER_EMAIL}}',
      'Content-Type': 'application/json',
    };
  ...
  it('should replace all allowed user field placeholders', () => {
    ...
    const headers = {
      'X-User-ID': '{{TEZGPT_USER_ID}}',
      'X-User-Name': '{{TEZGPT_USER_NAME}}',
      'X-User-Username': '{{TEZGPT_USER_USERNAME}}',
      'X-User-Email': '{{TEZGPT_USER_EMAIL}}',
      'X-User-Provider': '{{TEZGPT_USER_PROVIDER}}',
      'X-User-Role': '{{TEZGPT_USER_ROLE}}',
      'X-User-GoogleId': '{{TEZGPT_USER_GOOGLEID}}',
      'X-User-FacebookId': '{{TEZGPT_USER_FACEBOOKID}}',
      'X-User-OpenIdId': '{{TEZGPT_USER_OPENIDID}}',
      'X-User-SamlId': '{{TEZGPT_USER_SAMLID}}',
      'X-User-LdapId': '{{TEZGPT_USER_LDAPID}}',
      'X-User-GithubId': '{{TEZGPT_USER_GITHUBID}}',
      'X-User-DiscordId': '{{TEZGPT_USER_DISCORDID}}',
      'X-User-AppleId': '{{TEZGPT_USER_APPLEID}}',
      'X-User-EmailVerified': '{{TEZGPT_USER_EMAILVERIFIED}}',
      'X-User-TwoFactorEnabled': '{{TEZGPT_USER_TWOFACTORENABLED}}',
      'X-User-TermsAccepted': '{{TEZGPT_USER_TERMSACCEPTED}}',
    };
  ...
  it('should handle multiple placeholders in one value', () => {
    ...
    const headers = {
      'X-Multi': 'User: {{TEZGPT_USER_ID}}, Env: ${TEST_API_KEY}, Custom: {{MY_CUSTOM}}',
    };
  ...
  it('should handle a mix of all types', () => {
    ...
    const headers = {
      'X-User': '{{TEZGPT_USER_ID}}',
      'X-Env': '${TEST_API_KEY}',
      'X-Custom': '{{MY_CUSTOM}}',
      'X-Multi': 'ID: {{TEZGPT_USER_ID}}, ENV: ${TEST_API_KEY}, CUSTOM: {{MY_CUSTOM}}',
      'X-Unknown': '{{NOT_A_REAL_PLACEHOLDER}}',
      'X-Empty': '',
      'X-Boolean': '{{TEZGPT_USER_EMAILVERIFIED}}',
    };
    const customVars = { MY_CUSTOM: 'custom-value' };
    const result = resolveHeaders({ headers, user, customUserVars: customVars });
  ...
  it('should process TEZGPT_BODY placeholders', () => {
    const body = {
      conversationId: 'conv-123',
      parentMessageId: 'parent-456',
      messageId: 'msg-789',
    };
    const headers = { 'X-Conversation': '{{TEZGPT_BODY_CONVERSATIONID}}' };
    const result = resolveHeaders({ headers, body });
    expect(result['X-Conversation']).toBe('conv-123');
  });

  it('should not resolve env vars introduced via TEZGPT_BODY placeholders', () => {
    const body = {
    ...
    const headers = {
      'X-Conv': '{{TEZGPT_BODY_CONVERSATIONID}}',
      'X-Parent': '{{TEZGPT_BODY_PARENTMESSAGEID}}',
      'X-Msg': '{{TEZGPT_BODY_MESSAGEID}}',
    };
    const result = resolveHeaders({ headers, body });
  ...
  it('should not resolve env vars introduced via TEZGPT_USER placeholders', () => {
    const user = createTestUser({ name: '${TEST_API_KEY}' });
    const headers = { 'X-Name': '{{TEZGPT_USER_NAME}}' };
    const result = resolveHeaders({ headers, user });
  ...
  describe('non-string header values (type guard tests)', () => {
    ...
    it('should handle numeric values with placeholders', () => {
      ...
      const headers = {
        ...
        'X-String-With-Placeholder': '{{TEZGPT_USER_ID}}',
      };
      const result = resolveHeaders({ headers, user });
    ...
    it('should not crash when calling includes on non-string body field values', () => {
      ...
      const headers = {
        'X-Conv-Id': '{{TEZGPT_BODY_CONVERSATIONID}}',
        'X-Number': 999 as unknown as string,
      };
...
describe('resolveNestedObject', () => {
  ...
  it('should process strings in arrays', () => {
    const user = { id: 'user-123' };
    const obj = {
      headers: ['Authorization: Bearer ${TEST_API_KEY}', 'X-User-Id: {{TEZGPT_USER_ID}}'],
      values: [1, 2, 3],
      mixed: ['string', 42, true, '{{TEZGPT_USER_ID}}'],
    };
  ...
  it('should handle body placeholders in nested objects', () => {
    ...
    const obj = {
      metadata: {
        conversation: '{{TEZGPT_BODY_CONVERSATIONID}}',
        parent: '{{TEZGPT_BODY_PARENTMESSAGEID}}',
        count: 5,
      },
    };
  ...
  it('should handle arrays of objects', () => {
    ...
    const obj = {
      items: [
        { name: 'item1', user: '{{TEZGPT_USER_ID}}', count: 1 },
        { name: 'item2', user: '{{TEZGPT_USER_ID}}', count: 2 },
      ],
    };
  ...
  it('should not modify the original object', () => {
    ...
    expect(originalObj.thinking.user_id).toBe('{{TEZGPT_USER_ID}}');
  });
});
...
describe('processMCPEnv', () => {
  ...
  it('should not process user-controlled placeholders in outbound proxy', () => {
    ...
    const options: MCPOptions = {
      ...
      url: '${MCP_SERVER_URL}/sse',
      proxy:
        'http://proxy.example.com/{{CUSTOM_PROXY_PATH}}/{{TEZGPT_USER_ID}}/{{TEZGPT_BODY_MESSAGEID}}',
    };
  ...
  it('should process user field placeholders in all fields', () => {
    ...
    const options: MCPOptions = {
      ...
      env: {
        USER_ID: '{{TEZGPT_USER_ID}}',
        USER_EMAIL: '{{TEZGPT_USER_EMAIL}}',
        USER_ROLE: '{{TEZGPT_USER_ROLE}}',
      },
      args: ['--user', '{{TEZGPT_USER_USERNAME}}', '--id', '{{TEZGPT_USER_ID}}'],
    };
  ...
  it('should process body placeholders in all fields', () => {
    ...
    const options: MCPOptions = {
      type: 'streamable-http',
      url: 'https://api.example.com/conversations/{{TEZGPT_BODY_CONVERSATIONID}}',
      headers: {
        'X-Parent-Message': '{{TEZGPT_BODY_PARENTMESSAGEID}}',
        'X-Message-Id': '{{TEZGPT_BODY_MESSAGEID}}',
      },
    };
  ...
  it('should handle mixed placeholders in OAuth configuration', () => {
    ...
    const options: MCPOptions = {
      ...
      oauth: {
        ...
        client_id: '${OAUTH_CLIENT_ID}',
        client_secret: '${OAUTH_CLIENT_SECRET}',
        scope: 'user:{{TEZGPT_USER_ID}} conversation:{{TEZGPT_BODY_CONVERSATIONID}}',
        redirect_uri: 'http://localhost:3000/user/{{TEZGPT_USER_EMAIL}}/callback',
      },
    };
  ...
  it('should handle all placeholder types in a single value', () => {
    ...
    const options: MCPOptions = {
      ...
      env: {
        COMPLEX_VALUE:
          'User: {{TEZGPT_USER_ID}}, Custom: {{CUSTOM_VAR}}, Body: {{TEZGPT_BODY_CONVERSATIONID}}, Env: ${TEST_API_KEY}',
      },
    };
  ...
  describe('non-string values (type guard tests)', () => {
    ...
    it('should not crash with numeric body field values', () => {
      ...
      const options: MCPOptions = {
        ...
        args: [],
        env: {
          CONV_ID: '{{TEZGPT_BODY_CONVERSATIONID}}',
          PORT: 8080 as unknown as string,
        },
      };
  ...
  describe('dbSourced flag', () => {
    ...
    it('should NOT resolve {{TEZGPT_USER_*}} when dbSourced is true', () => {
      const user = createTestUser({ id: 'user-123', email: 'test@example.com' });
      const options: MCPOptions = {
        type: 'streamable-http',
        url: 'https://api.example.com',
      ...
      if (isStreamableHTTPOptions(result)) {
        expect(result.headers?.['X-User-Id']).toBe('{{TEZGPT_USER_ID}}');
        expect(result.headers?.['X-User-Email']).toBe('{{TEZGPT_USER_EMAIL}}');
      } else {
        throw new Error('Expected streamable-http options');
      }
    });

    it('should NOT resolve {{TEZGPT_OPENID_*}} when dbSourced is true', () => {
      const user = {
      ...
      const options: MCPOptions = {
        ...
        url: 'https://api.example.com',
        headers: {
          Authorization: 'Bearer {{TEZGPT_OPENID_ACCESS_TOKEN}}',
        },
      };
      ...
      if (isStreamableHTTPOptions(result)) {
        expect(result.headers?.Authorization).toBe('Bearer {{TEZGPT_OPENID_ACCESS_TOKEN}}');
      } else {
        throw new Error('Expected streamable-http options');
      }
    });

    it('should NOT resolve {{TEZGPT_BODY_*}} when dbSourced is true', () => {
      const body = {
        conversationId: 'conv-123',
        parentMessageId: 'parent-456',
        messageId: 'msg-789',
      ...
      const options: MCPOptions = {
        ...
        url: 'https://api.example.com',
        headers: {
          'X-Conversation': '{{TEZGPT_BODY_CONVERSATIONID}}',
        },
      };
      ...
      if (isStreamableHTTPOptions(result)) {
        expect(result.headers?.['X-Conversation']).toBe('{{TEZGPT_BODY_CONVERSATIONID}}');
      } else {
        throw new Error('Expected streamable-http options');
      }
    });
    ...
    it('should resolve customUserVars but block all other placeholders when dbSourced is true', () => {
      ...
      const options: MCPOptions = {
        ...
        headers: {
          ...
          'X-User-Id': '{{TEZGPT_USER_ID}}',
          'X-Body': '{{TEZGPT_BODY_CONVERSATIONID}}',
        },
      };
      ...
      if (isStreamableHTTPOptions(result)) {
        ...
        expect(result.headers?.['X-User-Id']).toBe('{{TEZGPT_USER_ID}}');
        expect(result.headers?.['X-Body']).toBe('{{TEZGPT_BODY_CONVERSATIONID}}');
...
describe('resolveHeaders stripUnresolved', () => {
  const templateHeaders = {
    'X-OpenID-Id': '{{TEZGPT_USER_OPENIDID}}',
    'X-User-Id': '{{TEZGPT_USER_ID}}',
    'Content-Type': 'application/json',
  };
  ...
  it('strips placeholders for fields the user lacks while substituting present fields', () => {
    ...
    const result = resolveHeaders({
      headers: { ...templateHeaders, 'X-Email': '{{TEZGPT_USER_EMAIL}}' },
      user: localUser,
      stripUnresolved: true,
    });
  ...
  it('strips only the placeholder within a composite header value', () => {
    const result = resolveHeaders({
      headers: { Authorization: 'Bearer {{TEZGPT_USER_OPENIDID}}' },
      stripUnresolved: true,
    });
  ...
  it('strips body placeholders when no body context is available', () => {
    const result = resolveHeaders({
      headers: { 'X-Convo': '{{TEZGPT_BODY_CONVERSATIONID}}' },
      user: { id: 'user-123' },
      stripUnresolved: true,
    });
  ...
  it('strips OpenID token placeholders when no valid token is available', () => {
    const result = resolveHeaders({
      headers: {
        'X-Access': '{{TEZGPT_OPENID_ACCESS_TOKEN}}',
        'X-Token': '{{TEZGPT_OPENID_TOKEN}}',
      },
      user: { id: 'user-123' },
      stripUnresolved: true,
    });
  ...
  it('leaves unknown and non-resolvable placeholders untouched', () => {
    const result = resolveHeaders({
      headers: {
        'X-Typo': '{{TEZGPT_USER_NONEXISTENT}}',
        'X-Graph': '{{TEZGPT_GRAPH_ACCESS_TOKEN}}',
        'X-Custom': '{{MY_CUSTOM_VAR}}',
      },
      stripUnresolved: true,
    });

    expect(result['X-Typo']).toBe('{{TEZGPT_USER_NONEXISTENT}}');
    expect(result['X-Graph']).toBe('{{TEZGPT_GRAPH_ACCESS_TOKEN}}');
  ...
  it('preserves unresolved placeholders by default (staged flows resolve later)', () => {
    ...
    expect(result['X-OpenID-Id']).toBe('{{TEZGPT_USER_OPENIDID}}');
    expect(result['X-User-Id']).toBe('{{TEZGPT_USER_ID}}');
  });
});
