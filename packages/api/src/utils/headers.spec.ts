...
describe('resolveConfigHeaders', () => {
    ...
    const llmConfig = {
      clientOptions: {
        defaultHeaders: {
          ...
          'cf-aig-metadata': '{"conversation_id":"{{TEZGPT_BODY_CONVERSATIONID}}"}',
        },
      },
    } as unknown as RunLLMConfig;
  ...
  it('resolves each header map only once across repeated calls (idempotent under reuse)', () => {
    ...
    const llmConfig = {
      configuration: { defaultHeaders: { 'X-Name': '{{TEZGPT_USER_NAME}}' } },
    } as unknown as RunLLMConfig;
  ...
  it('never forwards unresolved user placeholders when user context is missing (issue #14580)', () => {
    const llmConfig = {
      configuration: {
        defaultHeaders: {
          'X-TezGPT-User': '{{TEZGPT_USER_OPENIDID}}',
          'X-Conversation-Id': '{{TEZGPT_BODY_CONVERSATIONID}}',
        },
      },
    } as unknown as RunLLMConfig;
  ...
  it('strips placeholders for fields the resolved user lacks', () => {
    const llmConfig = {
      configuration: {
        defaultHeaders: { 'X-TezGPT-User': '{{TEZGPT_USER_OPENIDID}}' },
      },
    } as unknown as RunLLMConfig;

    resolveConfigHeaders({ llmConfig, user, body });

    expect(llmConfig.configuration?.defaultHeaders).toEqual({ 'X-TezGPT-User': '' });
  });
});
