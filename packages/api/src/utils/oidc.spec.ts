...
describe('OpenID Token Utilities', () => {
  ...
  describe('processOpenIDPlaceholders', () => {
    it('should replace TEZGPT_OPENID_TOKEN with access token', () => {
      const tokenInfo = {
        accessToken: 'access-token-value',
        idToken: 'id-token-value',
        userId: 'oidc-sub-456',
      ...
      const input = 'Authorization: Bearer {{TEZGPT_OPENID_TOKEN}}';
      const result = processOpenIDPlaceholders(input, tokenInfo);

      expect(result).toBe('Authorization: Bearer access-token-value');
    });

    it('should replace TEZGPT_OPENID_ACCESS_TOKEN with access token', () => {
      const tokenInfo = {
        accessToken: 'access-token-value',
        userId: 'oidc-sub-456',
      };

      const input = 'Token: {{TEZGPT_OPENID_ACCESS_TOKEN}}';
      const result = processOpenIDPlaceholders(input, tokenInfo);

      expect(result).toBe('Token: access-token-value');
    });

    it('should replace TEZGPT_OPENID_ID_TOKEN with id token', () => {
      const tokenInfo = {
        idToken: 'id-token-value',
        userId: 'oidc-sub-456',
      };

      const input = 'ID Token: {{TEZGPT_OPENID_ID_TOKEN}}';
      const result = processOpenIDPlaceholders(input, tokenInfo);

      expect(result).toBe('ID Token: id-token-value');
    });

    it('should replace TEZGPT_OPENID_USER_ID with user id', () => {
      const tokenInfo = {
        userId: 'oidc-sub-456',
      };

      const input = 'User: {{TEZGPT_OPENID_USER_ID}}';
      const result = processOpenIDPlaceholders(input, tokenInfo);

      expect(result).toBe('User: oidc-sub-456');
    });

    it('should replace TEZGPT_OPENID_USER_EMAIL with user email', () => {
      const tokenInfo = {
        userEmail: 'test@example.com',
        userId: 'oidc-sub-456',
      };

      const input = 'Email: {{TEZGPT_OPENID_USER_EMAIL}}';
      const result = processOpenIDPlaceholders(input, tokenInfo);

      expect(result).toBe('Email: test@example.com');
    });

    it('should replace TEZGPT_OPENID_USER_NAME with user name', () => {
      const tokenInfo = {
        userName: 'Test User',
        userId: 'oidc-sub-456',
      };

      const input = 'Name: {{TEZGPT_OPENID_USER_NAME}}';
      const result = processOpenIDPlaceholders(input, tokenInfo);
    ...
    it('should replace multiple placeholders in a single string', () => {
      ...
      const input =
        'Authorization: Bearer {{TEZGPT_OPENID_TOKEN}}, ID: {{TEZGPT_OPENID_ID_TOKEN}}, User: {{TEZGPT_OPENID_USER_ID}}';
      const result = processOpenIDPlaceholders(input, tokenInfo);
    ...
    it('should replace empty string when token field is undefined', () => {
      ...
      const input =
        'Access: {{TEZGPT_OPENID_TOKEN}}, ID: {{TEZGPT_OPENID_ID_TOKEN}}, User: {{TEZGPT_OPENID_USER_ID}}';
      const result = processOpenIDPlaceholders(input, tokenInfo);
    ...
    it('should handle all placeholder types in one value', () => {
      ...
      const input = `
        Authorization: Bearer {{TEZGPT_OPENID_TOKEN}}
        ID Token: {{TEZGPT_OPENID_ID_TOKEN}}
        Access Token (alt): {{TEZGPT_OPENID_ACCESS_TOKEN}}
        User ID: {{TEZGPT_OPENID_USER_ID}}
        User Email: {{TEZGPT_OPENID_USER_EMAIL}}
        User Name: {{TEZGPT_OPENID_USER_NAME}}
        Expires: {{TEZGPT_OPENID_EXPIRES_AT}}
      `;
    ...
    it('should handle case-sensitive placeholders', () => {
      ...
      const input = 'Token: {{tezgpt_openid_token}}';
      const result = processOpenIDPlaceholders(input, tokenInfo);

      expect(result).toBe('Token: {{tezgpt_openid_token}}');
    });
    ...
    it('should handle multiple occurrences of the same placeholder', () => {
      ...
      const input =
        'Primary: {{TEZGPT_OPENID_TOKEN}}, Secondary: {{TEZGPT_OPENID_TOKEN}}, Backup: {{TEZGPT_OPENID_TOKEN}}';
      const result = processOpenIDPlaceholders(input, tokenInfo);
    ...
    it('should return original value when tokenInfo is null', () => {
      ...
      expect(result).toBe('Authorization: Bearer {{TEZGPT_OPENID_TOKEN}}');
    });
  ...
  describe('Integration: Full OpenID Token Flow', () => {
    it('should extract, validate, and process tokens correctly', () => {
      ...
      const input =
        'Authorization: Bearer {{TEZGPT_OPENID_TOKEN}}, User: {{TEZGPT_OPENID_USER_ID}}';
      const result = processOpenIDPlaceholders(input, tokenInfo!);
      expect(result).toContain('Authorization: Bearer access-token-value');
      expect(result).toContain('User:');
    });

    it('should resolve TEZGPT_OPENID_ID_TOKEN and TEZGPT_OPENID_ACCESS_TOKEN to different values', () => {
      const user: Partial<IUser> = {
        id: 'user-123',
        provider: 'openid',
        openidId: 'oidc-sub-456',
      ...
      const input = 'ACCESS={{TEZGPT_OPENID_ACCESS_TOKEN}}, ID={{TEZGPT_OPENID_ID_TOKEN}}';
      const result = processOpenIDPlaceholders(input, tokenInfo!);
