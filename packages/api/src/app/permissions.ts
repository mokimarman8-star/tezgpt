import { logger, tenantStorage, SYSTEM_TENANT_ID } from '@tezgpt/data-schemas';
import {
  SystemRoles,
  Permissions,
  roleDefaults,
...
import type { IRole, AppConfig } from '@tezgpt/data-schemas';
...
export async function updateInterfacePermissions({
  ...
  // 1. Explicit user configuration (from tezgpt.yaml)
  ...
  for (const roleName of [SystemRoles.USER, SystemRoles.ADMIN]) {
     ...
     * unless explicitly configured otherwise in tezgpt.yaml.
     */
