...
import { logger, webSearchKeys } from '@tezgpt/data-schemas';
import { Constants, extractVariableName } from 'tezgpt-data-provider';
import type { TCustomConfig } from 'tezgpt-data-provider';
import type { AppConfig } from '@tezgpt/data-schemas';
...
const deprecatedVariables = [
  {
    key: 'CHECK_BALANCE',
    description:
      'Please use the `balance` field in the `tezgpt.yaml` config file instead.\nMore info: https://tezgpt.app/docs/configuration/tezgpt_yaml/object_structure/balance#overview',
  },
  {
    key: 'START_BALANCE',
    description:
...
function checkPasswordReset() {
  ...
  if (!emailEnabled && passwordResetAllowed) {
    logger.warn(
      ...
      https://www.tezgpt.app/docs/configuration/authentication/email
...
export function checkVariables(): void {
  ...
  if (legacyNames.length > 0) {
    ...
    https://www.tezgpt.app/toolkit/creds_generator
...
Latest version: ${Constants.CONFIG_VERSION}
      ...
      https://www.tezgpt.app/changelog\n\n`,
    );
  }
}
    ...
    if (typeof value === 'string') {
      ...
      if (varName) {
        ...
        logger.warn(
          ...
          This is incorrect! You should use environment variable references in your tezgpt.yaml file, such as:
          ${key}: "\${YOUR_ENV_VAR_NAME}"
          
          Then set the actual API key in your .env file or environment variables.
          
          More info: https://www.tezgpt.app/docs/configuration/tezgpt_yaml/web_search`,
        );
      }
    }
  });
