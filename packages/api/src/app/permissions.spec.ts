import { loadDefaultInterface } from '@tezgpt/data-schemas';
import { SystemRoles, Permissions, PermissionTypes, roleDefaults } from 'tezgpt-data-provider';
import type { TConfigDefaults, TCustomConfig } from 'tezgpt-data-provider';
...
describe('updateInterfacePermissions - permissions', () => {
  ...
  it('should preserve existing SHARE/SHARE_PUBLIC values when using boolean config (regression test)', async () => {
    ...
    // Config uses boolean (not object), simulating `agents: true` in tezgpt.yaml
    const config = {
      interface: {
