...
describe('ServerConfigsCacheInMemory Integration Tests', () => {
  ...
  describe('credential placeholders in YAML configs', () => {
    it('should preserve TEZGPT_OPENID placeholders (admin configs are trusted)', async () => {
      ...
      expect(retrievedWithHeaders?.headers?.Authorization).toBe(
        'Bearer {{TEZGPT_OPENID_ACCESS_TOKEN}}',
      );
      expect(retrievedWithHeaders?.headers?.['X-User-Id']).toBe('{{TEZGPT_OPENID_USER_ID}}');
    });

    it('should preserve TEZGPT_USER placeholders (admin configs are trusted)', async () => {
      const adminConfig: ParsedServerConfig & { headers?: Record<string, string> } = {
        type: 'sse',
      ...
      expect(retrievedWithHeaders?.headers?.['X-User-Email']).toBe('{{TEZGPT_USER_EMAIL}}');
      expect(retrievedWithHeaders?.headers?.['X-User-Name']).toBe('{{TEZGPT_USER_NAME}}');
      expect(retrievedWithHeaders?.headers?.['X-User-Id']).toBe('{{TEZGPT_USER_ID}}');
    });
  });
});
