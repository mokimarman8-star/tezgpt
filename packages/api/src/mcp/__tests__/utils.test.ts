...
describe('splitMCPToolKey', () => {
  ...
  it('should resolve a raw tool name that itself contains the delimiter substring by using the last occurrence', () => {
    ...
    // before TezGPT appends its own "_mcp_gitlab" suffix. A naive
    ...
    // "gitlab"). See https://github.com/tezgpt-ai/TezGPT/issues/14440
...
describe('redactServerSecrets', () => {
  ...
  it('should expose request-scoped behavior without exposing placeholder-bearing fields', () => {
    const config: ParsedServerConfig = {
      ...
      source: 'yaml',
      headers: { 'X-Conversation': '{{TEZGPT_BODY_CONVERSATIONID}}' },
    };
  ...
  it('should omit request-scoped metadata for ordinary and unsupported BODY placeholders', () => {
    ...
    expect(
      redactServerSecrets({
        type: 'streamable-http',
        url: 'https://example.com/{{TEZGPT_BODY_TENANT}}/mcp',
        source: 'yaml',
      }).requestScoped,
    ).toBeUndefined();
  });
...
describe('hasRuntimeContextPlaceholders', () => {
  it('detects trusted runtime placeholders across connection fields', () => {
    expect(
      hasRuntimeContextPlaceholders({
        source: 'config',
        url: 'https://example.com/{{TEZGPT_BODY_MESSAGEID}}/mcp',
        headers: {
          Authorization: 'Bearer {{TEZGPT_OPENID_ID_TOKEN}}',
          'X-Graph-Access-Token': '{{TEZGPT_GRAPH_ACCESS_TOKEN}}',
        },
      }),
    ).toBe(true);
  });
...
describe('hasRuntimeUrlPlaceholders', () => {
  it('detects trusted runtime placeholders in the server URL', () => {
    expect(
      hasRuntimeUrlPlaceholders({
        source: 'yaml',
        url: 'https://example.com/users/{{TEZGPT_USER_USERNAME}}/mcp',
      }),
    ).toBe(true);
  });
...
describe('hasRuntimeBodyPlaceholders', () => {
  it('detects trusted runtime BODY placeholders across connection fields', () => {
    expect(
      hasRuntimeBodyPlaceholders({
        source: 'yaml',
        url: 'https://example.com/conversations/{{TEZGPT_BODY_CONVERSATIONID}}/mcp',
      }),
    ).toBe(true);
    ...
    expect(
      hasRuntimeBodyPlaceholders({
        source: 'config',
        headers: {
          'X-Message': '{{TEZGPT_BODY_MESSAGEID}}',
        },
      }),
    ).toBe(true);
  });
  ...
  it('ignores BODY and USER literals in plugin-sourced configs', () => {
    const config = {
      ...
      url: 'https://example.com/users/{{TEZGPT_USER_ID}}/mcp',
      headers: {
...
describe('requiresEphemeralUserConnection', () => {
  ...
  it('returns true when BODY placeholders affect connection fields', () => {
    expect(
      requiresEphemeralUserConnection({
        source: 'yaml',
        url: 'https://example.com/messages/{{TEZGPT_BODY_MESSAGEID}}/mcp',
      }),
    ).toBe(true);
  });
  ...
  it('does not treat OpenID token placeholders as request-scoped by themselves', () => {
    expect(
      requiresEphemeralUserConnection({
        source: 'yaml',
        args: ['--id-token={{TEZGPT_OPENID_ID_TOKEN}}'],
      }),
    ).toBe(false);
