 ...
 * 3. DB-sourced servers do NOT resolve `{{TEZGPT_USER_*}}` placeholders.
 * 4. DB-sourced servers do NOT resolve `{{TEZGPT_BODY_*}}` placeholders.
...
describe('dbSourced header security – integration', () => {
  ...
  it('DB-sourced: does NOT resolve {{TEZGPT_USER_*}} placeholders', async () => {
    ...
    const captured = server.getLastHeaders();
    expect(captured['x-user-id']).toBe('{{TEZGPT_USER_ID}}');
    expect(captured['x-user-email']).toBe('{{TEZGPT_USER_EMAIL}}');
    expect(captured['x-user-id']).not.toBe('user-secret-id');
  });

  it('DB-sourced: does NOT resolve {{TEZGPT_BODY_*}} placeholders', async () => {
    const body = {
    ...
    const captured = server.getLastHeaders();
    expect(captured['x-conv-id']).toBe('{{TEZGPT_BODY_CONVERSATIONID}}');
