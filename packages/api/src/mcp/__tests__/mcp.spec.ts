...
import type { TUser } from 'tezgpt-data-provider';
...
describe('Environment Variable Extraction (MCP)', () => {
  ...
  describe('processMCPEnv', () => {
    ...
    it('should ensure different users with same starting config get separate values', () => {
      ...
      expect(baseConfig.headers?.['User-Id']).toBe('{{TEZGPT_USER_ID}}');
    ...
    it('should process dynamic user fields in headers', () => {
      ...
      const options: MCPOptions = {
        ...
        headers: {
          ...
          'User-Name': '{{TEZGPT_USER_USERNAME}}',
          OpenID: '{{TEZGPT_USER_OPENIDID}}',
          'Google-ID': '{{TEZGPT_USER_GOOGLEID}}',
          'Email-Verified': '{{TEZGPT_USER_EMAILVERIFIED}}',
    ...
    it('should process user fields in URL', () => {
      ...
      const options: MCPOptions = {
        type: 'sse',
        url: 'https://example.com/api/{{TEZGPT_USER_USERNAME}}/stream',
      };
    ...
    it('should handle boolean user fields', () => {
      ...
      const options: MCPOptions = {
        ...
        headers: {
          'Email-Verified': '{{TEZGPT_USER_EMAILVERIFIED}}',
          'Two-Factor': '{{TEZGPT_USER_TWOFACTORENABLED}}',
          'Terms-Accepted': '{{TEZGPT_USER_TERMSACCEPTED}}',
        },
      };
    ...
    it('should not process sensitive fields like password', () => {
      ...
      const options: MCPOptions = {
        ...
        headers: {
          'User-Email': '{{TEZGPT_USER_EMAIL}}',
          'User-Password': '{{TEZGPT_USER_PASSWORD}}', // This should not be processed
        },
      };
      ...
      expect('headers' in result && result.headers).toEqual({
        'User-Email': 'test@example.com',
        'User-Password': '{{TEZGPT_USER_PASSWORD}}', // Unchanged
      });
    });
    ...
    it('should support both id and _id properties for TEZGPT_USER_ID', () => {
      ...
      expect('headers' in result2 && result2.headers?.['User-Id']).toBe('{{TEZGPT_USER_ID}}');
    ...
    it('should process customUserVars in URL field', () => {
      ...
      const options: MCPOptions = {
        type: 'websocket',
        url: 'wss://example.com/{{TENANT_ID}}/api/{{API_VERSION}}?user={{TEZGPT_USER_ID}}&key=${TEST_API_KEY}',
      };
    ...
    it('should prioritize customUserVars over user fields and system env vars if placeholders are the same (though not recommended)', () => {
      ...
      process.env.TEZGPT_USER_EMAIL = 'system-email-should-be-overridden';
      const user = createTestUser({ email: 'user-email-should-be-overridden' });
      const customUserVars = {
        TEZGPT_USER_EMAIL: 'custom-email-wins',
      };
      const options: MCPOptions = {
        type: 'sse',
        url: 'https://example.com/api',
        headers: {
          'Test-Email': '{{TEZGPT_USER_EMAIL}}', // Placeholder that could match custom, user, or system
        },
      };
      ...
      // Clean up env var
      delete process.env.TEZGPT_USER_EMAIL;
    });
    ...
    it('should handle placeholders with no matching customUserVars (falling back to user/system vars)', () => {
      ...
      const options: MCPOptions = {
        ...
        url: 'https://example.com/api',
        headers: {
          'User-Email-Header': '{{TEZGPT_USER_EMAIL}}', // Should use user.email
    ...
    it('should correctly process a mix of all variable types', () => {
      ...
      const obj = {
        type: 'streamable-http' as const,
        url: 'https://{{CUSTOM_ENDPOINT_ID}}.example.com/users/{{TEZGPT_USER_USERNAME}}',
        headers: {
        ...
        env: {
          ...
          USER_HOME_DIR: '/home/{{TEZGPT_USER_USERNAME}}',
    ...
    it('should process GitHub MCP server configuration with PAT_TOKEN placeholder', () => {
      ...
      // Simulate the GitHub MCP server configuration from tezgpt.yaml
      const options: MCPOptions = {
        type: 'streamable-http',
    ...
    it('should leave {{TEZGPT_GRAPH_ACCESS_TOKEN}} unchanged (resolved by preProcessGraphTokens)', () => {
      const user = createTestUser({ id: 'user-123' });
      const options: MCPOptions = {
        type: 'sse',
        url: 'https://example.com',
        headers: {
          Authorization: 'Bearer {{TEZGPT_GRAPH_ACCESS_TOKEN}}',
          'X-User-Id': '{{TEZGPT_USER_ID}}',
        },
      };
  ...
  describe('preProcessGraphTokens and processMCPEnv working in tandem', () => {
    ...
    it('should resolve graph tokens in env and user placeholders in headers', async () => {
      ...
      const options: MCPOptions = {
        type: 'stdio',
        command: 'node',
        args: ['mcp-server.js', '--user', '{{TEZGPT_USER_USERNAME}}'],
        env: {
          GRAPH_ACCESS_TOKEN: '{{TEZGPT_GRAPH_ACCESS_TOKEN}}',
    ...
    it('should resolve graph tokens in args and oauth fields before processMCPEnv', async () => {
      ...
      const options: MCPOptions = {
        type: 'stdio',
        command: 'node',
        args: ['server.js', '--graph-token={{TEZGPT_GRAPH_ACCESS_TOKEN}}'],
        oauth: {
          client_id: 'client-id',
          client_secret: '{{TEZGPT_GRAPH_ACCESS_TOKEN}}',
          scope: 'profile {{TEZGPT_GRAPH_ACCESS_TOKEN}}',
        },
      };
    ...
    it('should resolve graph tokens in URL alongside other placeholders', async () => {
      ...
      const options: MCPOptions = {
        type: 'sse',
        url: 'https://{{TENANT_ID}}.example.com/api?token={{TEZGPT_GRAPH_ACCESS_TOKEN}}&user={{TEZGPT_USER_ID}}',
      };
