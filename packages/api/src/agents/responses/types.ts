...
export interface HostedTool {
  type: string; // e.g., 'tezgpt:web_search'
  [key: string]: unknown;
}
...
/** Attachment content types for TezGPT extensions */
export interface TezGPTAttachmentContent {
  /** File ID in TezGPT storage */
  file_id?: string;
  /** Original filename */
  filename?: string;
  /** MIME type */
...
/**
 * TezGPT attachment event - custom streaming event for file/image attachments
 * Follows Open Responses extension pattern with tezgpt: prefix
 */
export interface TezGPTAttachmentEvent extends BaseEvent {
  type: 'tezgpt:attachment';
  /** The attachment data */
  attachment: TezGPTAttachmentContent;
...
/** Union of all streaming events (including TezGPT extensions) */
export type ResponseEvent =
  ...
  | ReasoningDoneEvent
  | ErrorEvent
  // TezGPT extensions (prefixed per Open Responses spec)
  | TezGPTAttachmentEvent;
