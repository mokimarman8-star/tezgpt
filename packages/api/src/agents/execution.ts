...
export function getCodeExecutionBaseUrl(profile: CodeExecutionProfile): string {
  ...
  const baseUrl = process.env.TEZGPT_CODE_BASEURL_STATEFUL?.trim().replace(/\/+$/, '');
  if (baseUrl) {
    return baseUrl;
  }
  throw new Error(
    'Stateful code execution is enabled for this agent, but TEZGPT_CODE_BASEURL_STATEFUL is not configured.',
  );
}
