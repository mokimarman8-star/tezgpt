...
import type { TCheckpointerConfig } from 'tezgpt-data-provider';
import type { RunnableConfig } from '@langchain/core/runnables';

/**
 * LangGraph reserves `checkpoint_ns` for nested graph namespaces and forcibly
 * resets a non-empty value to `''` for every root invocation. Carry TezGPT's
 ...
 */
export const TEZGPT_CHECKPOINT_NAMESPACE_KEY = '__tezgpt_checkpoint_ns';

const CHECKPOINT_NAMESPACE_SEPARATOR = '|';

function generationCheckpointNamespace(config: RunnableConfig): string | undefined {
  const value = config.configurable?.[TEZGPT_CHECKPOINT_NAMESPACE_KEY];
...
function fromStorageCheckpointConfig(
  ...
  return {
    ...
    configurable: {
      ...
      [TEZGPT_CHECKPOINT_NAMESPACE_KEY]: generationNamespace,
    },
  };
}
 ...
 *
 * This is the seam between TezGPT and LangGraph's checkpoint machinery. A run
 ...
 *
 * For TezGPT's agent graph (standard `Annotation`/`MessagesAnnotation` channels, no
...
export class LazyMongoSaver extends MongoDBSaver {
   ...
   * touching the saver. Map TezGPT's private generation key into Mongo's
 ...
 *
 * @param threadId - the LangGraph `thread_id` (TezGPT's conversationId).
 ...
 *
 * @param threadIds - LangGraph `thread_id`s (TezGPT conversationIds); falsy entries skipped.
 */
