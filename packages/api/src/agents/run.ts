...
import type { AppConfig, IUser } from '@tezgpt/data-schemas';
 ...
 * `tezgpt.yaml` flows verbatim to the agents SDK, which only knows a fixed
...
function resolveSummarizationProvider(
  ...
  try {
     ...
     * `{{TEZGPT_BODY_PARENTMESSAGEID}}`) before handing them to
...
>): Promise<Run<IState>> {
  ...
  /**
   * Deployment-plugin hooks (Agent Plugins `ai.tezgpt/hooks/hooks.json`)
  ...
  const runConfig = {
    ...
    // Stream circuit breakers (tezgpt.yaml endpoints.agents.maxToolCallArgBytes /
