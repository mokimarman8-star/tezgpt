...
import { openAIBaseSchema, googleBaseSchema, anthropicBaseSchema } from 'tezgpt-data-provider';
import type { Agents, TToolApprovalPolicy } from 'tezgpt-data-provider';
...
export interface ToolApprovalPolicyLayers {
  /**
   * App/endpoint policy — `endpoints.agents.toolApproval` from tezgpt.yaml.
 ...
 * (`TezGPTCheckpointSaver`). Otherwise the SDK installs a process-local
...
/**
 * Map a TezGPT tool-approval policy to the SDK's `ToolPolicyConfig`.
 *
 * Returns `undefined` when there's nothing to configure (so the SDK's own
 * defaults apply). The `enabled` field is TezGPT-only and stripped here —
