...
import { Constants, FileSources } from 'tezgpt-data-provider';
import { agentSchema, createMethods } from '@tezgpt/data-schemas';
import type {
  Agent as TezGPTAgent,
...
const deps: LoadAgentDeps = {
  getAgent: (searchParameter) => getAgent(searchParameter) as Promise<TezGPTAgent | null>,
    ...
    const result = await loadAddedAgent(
      {
        ...
        primaryAgent: { id: EPHEMERAL_AGENT_ID as string, tools: ['web_search'] } as TezGPTAgent,
      },
      deps,
    );
