import { encryptV3, logger } from '@tezgpt/data-schemas';
import {
  EModelEndpoint,
  FileSources,
  MAX_SUBAGENT_DEPTH,
...
import type { SummarizationConfig, TEndpoint } from 'tezgpt-data-provider';
import type { AppConfig } from '@tezgpt/data-schemas';
import { createRun } from '~/agents/run';

// Mock winston logger — `format` must be callable so @tezgpt/data-schemas
...
jest.mock('@tezgpt/data-schemas', () => ({
  ...jest.requireActual('@tezgpt/data-schemas'),
...
/**
 * Shape of summarization parameters used in tests. The TezGPT config
...
describe('Langfuse run config', () => {
  ...
  it('adds the explicit request tenant id to Langfuse trace metadata and tags', async () => {
    ...
    expect(callArgs.langfuse).toEqual({
      ...
      metadata: { 'tezgpt.tenant.id': 'tenant-1' },
  ...
  it('falls back to a full user tenant id for direct createRun callers', async () => {
    ...
    expect(callArgs.langfuse).toEqual({
      deterministicTraceId: true,
      metadata: { 'tezgpt.tenant.id': 'tenant-2' },
  ...
  it('adds tenant Langfuse credentials from tenant-scoped app config', async () => {
    ...
    expect(callArgs.langfuse).toEqual({
      ...
      metadata: { 'tezgpt.tenant.id': 'tenant-1' },
      tezgptTraceAttributes: {
        'tezgpt.langfuse.tenant_export.enabled': 'true',
        'tezgpt.langfuse.destination': 'eu',
      },
      tags: ['tenant:tenant-1'],
    });
  });
  ...
  it('routes tenant fanout traces to the configured tenant destination', async () => {
    ...
    expect(callArgs.langfuse).toMatchObject({
      ...
      tezgptTraceAttributes: {
        'tezgpt.langfuse.tenant_export.enabled': 'true',
        'tezgpt.langfuse.destination': 'us',
      },
    });
  });
  ...
  it('does not append a tenant route to baseUrl when fanout is disabled', async () => {
    ...
    expect(callArgs.langfuse).not.toMatchObject({
      ...
      tezgptTraceAttributes: expect.any(Object),
    });
  });
