...
import type { IMongoFile } from '@tezgpt/data-schemas';
import type { Agent } from 'tezgpt-data-provider';
...
// Mock logger — `format` must be a callable factory so @tezgpt/data-schemas
...
function createMocks(overrides?: {
  ...
  mockExtractTezGPTParams.mockReturnValue({
    resendFiles: false,
    maxContextTokens,
    modelOptions: { model },
  });
...
describe('initializeAgent — provider web_search precedence', () => {
  ...
  it('keeps Anthropic native web_search when TezGPT search is not selected', async () => {
    const { agent, req, res, loadTools, db } = createMocks({
  ...
  it('keeps TezGPT web_search definitions when native Anthropic search is not enabled', async () => {
    const { agent, req, res, loadTools, db } = createMocks({
      provider: Providers.ANTHROPIC,
  ...
  it('prefers TezGPT web_search when Anthropic native search is also enabled', async () => {
    const { agent, req, res, loadTools, db } = createMocks({
      provider: Providers.ANTHROPIC,
      providerTools: [nativeWebSearchTool],
      loadedToolDefinitions: [libreChatWebSearchDefinition],
  ...
  it('keeps Google native search when TezGPT web_search is not selected', async () => {
    const { agent, req, res, loadTools, db } = createMocks({
  ...
  it('prefers TezGPT web_search when Google native search is also enabled', async () => {
    const { agent, req, res, loadTools, db } = createMocks({
      provider: Providers.GOOGLE,
      providerTools: [nativeGoogleSearchTool],
      loadedToolDefinitions: [libreChatWebSearchDefinition],
  ...
  it('preserves the Google urlContext tool when TezGPT web_search is also enabled', async () => {
    /**
     * url_context is unrelated to web search, so the TezGPT web_search conflict
...
describe('initializeAgent — attachment scoping', () => {
  ...
  it('owner-scopes request file usage updates while preserving trusted tool files', async () => {
    ...
    mockExtractTezGPTParams.mockReturnValueOnce({
      resendFiles: true,
...
describe('initializeAgent — execute_code capability expansion', () => {
  ...
  it('routes code-file priming through the stateful profile before tools load', async () => {
    ...
    process.env.TEZGPT_CODE_BASEURL_STATEFUL = 'https://stateful-code.example.com/v1/';
    ...
    try {
      ...
      delete process.env.TEZGPT_CODE_BASEURL_STATEFUL;
    }
  });
...
describe('initializeAgent — code-generated file thread filter (regression)', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    mockExtractTezGPTParams.mockReset();
