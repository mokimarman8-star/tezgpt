 ...
 * interacting with TezGPT agents. The agent_id is passed as the "model"
 * parameter per OpenAI spec.
 *
 * Usage:
 * ```typescript
 * import { createAgentChatCompletion } from '@tezgpt/api';
 *
...
import { AgentCapabilities } from 'tezgpt-data-provider';
import type { StatefulCodeEnvironment } from 'tezgpt-data-provider';
...
/**
 * Agent type from tezgpt-data-provider
 */
interface Agent {
  id: string;
  name?: string;
...
/**
 * Convert OpenAI messages to TezGPT format
 */
