 ...
 *
 * These handlers convert TezGPT's internal graph events into OpenAI-compatible
...
/**
 * Step types from tezgpt-data-provider
 */
...
export class OpenAIRunStepHandler implements EventHandler {
  ...
  handle(_event: string, data: { stepDetails?: { type?: string } }): void {
    // Run step events are primarily for TezGPT UI, we use deltas for streaming
