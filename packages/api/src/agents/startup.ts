...
import { ApprovalEvents, StepEvents } from 'tezgpt-data-provider';
...
const SPAN_NAME = 'tezgpt.agent.startup';
...
export function createAgentStartupTelemetry(
  ...
  const mark = (milestone: AgentStartupMilestone): void => {
    ...
    if (tracingEnabled) {
      span.addEvent(milestone, {
        'tezgpt.agent.startup.elapsed_ms': elapsedMs,
      });
    }
    if (metricsEnabled) {
  ...
  const setStreamId = (streamId: string): void => {
    ...
    if (tracingEnabled) {
      span.setAttribute('tezgpt.stream.id', streamId);
    }
  };
  ...
  const end = (result: AgentStartupResult, error?: Error): void => {
    ...
    if (tracingEnabled) {
      span.setAttributes({
        'tezgpt.agent.startup.duration_ms': elapsedMilliseconds(),
        'tezgpt.agent.startup.milestones.count': milestones.size,
        'tezgpt.agent.startup.result': normalizedResult,
      });
    }
    if (metricsEnabled) {
