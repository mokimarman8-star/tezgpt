...
describe('resolveCodeExecutionContext', () => {
  const originalStatefulUrl = process.env.TEZGPT_CODE_BASEURL_STATEFUL;
  ...
  afterEach(() => {
    ...
    process.env.TEZGPT_CODE_BASEURL_STATEFUL = originalStatefulUrl;
  });
  ...
  it('fails closed when a stateful agent has no stateful endpoint', () => {
    ...
    expect(() => resolveCodeExecutionContext({ statefulSessions: true, userId: 'user-1' })).toThrow(
      'TEZGPT_CODE_BASEURL_STATEFUL is not configured',
    );
  });
