...
export interface PluginHookRuntimeContext {
  ...
  userId?: string;
  /**
   * Session working directory for the payload's `cwd`. TezGPT runs supply
 ...
 * This compatibility layer never launches plugin code in the TezGPT API process.
 */
...
export function createPluginHookPayload(
  ...
  switch (input.hook_event_name) {
    ...
    /** TezGPT steering seal; no Claude counterpart, so it is never mapped to a declaration. */
