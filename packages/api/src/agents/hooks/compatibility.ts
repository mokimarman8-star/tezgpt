...
export interface PluginHookToolInputTranslation {
  sourceEvent: string;
  targetEvent: HookEvent;
  /** TezGPT runtime name of the invoked tool. */
  toolName: string;
...
export interface PluginHookCapabilities {
  ...
  /** Maps a TezGPT runtime tool name back into the plugin's source namespace. */
...
/**
 * SessionStart lifecycle sources no TezGPT run-construction path emits.
...
const UNAVAILABLE_SESSION_SOURCES: Readonly<Record<string, string>> = Object.freeze({
  compact:
    'SessionStart source "compact" is unavailable because TezGPT PostCompact hook output cannot inject session context',
  clear:
    'SessionStart source "clear" is unavailable because TezGPT has no clear-conversation lifecycle path',
});
...
function getMatcherValidationIssue(
  ...
  if (targetEvent && !QUERY_EVENTS.has(targetEvent) && sourceEvent !== 'SessionStart') {
    return {
      ...
      severity: 'error',
      message: `${targetEvent} does not expose a matcher query in the TezGPT hook runtime`,
    };
  }
  if (matcher.length > MAX_PATTERN_LENGTH || hasNestedQuantifier(matcher)) {
    return {
      ...
      severity: 'error',
      message: 'Matcher exceeds the safe regex limits enforced by the TezGPT hook runtime',
    };
  }
  try {
    ...
    return [
      {
        ...
        severity: 'error',
        message: `${sourceEvent} has no equivalent TezGPT lifecycle event`,
      },
    ];
  }
    ...
    return [
      {
        ...
        severity: 'error',
        message:
          'SubagentStop is unavailable because the TezGPT hook input does not expose stop-hook state',
      },
    ];
  }
    ...
    return [
      {
        ...
        severity: 'error',
        message:
          'PermissionDenied is unavailable because the TezGPT hook output cannot request a retry',
      },
    ];
  }
...
function planMatcher(
        ...
        issues: [
          {
            ...
            severity: 'warning',
            message:
              'Wildcard SessionStart compatibility covers startup and resume; compact and clear never occur in TezGPT',
          },
        ],
      };
    }
      ...
      issues: [
        {
          ...
          severity: 'error',
          message: 'Plugin matcher namespaces require an explicit TezGPT query translation',
        },
      ],
    };
  }
  ...
  if (normalizedTranslation !== matcherSemantics.canonicalMatcher) {
    issues.push({
      ...
      severity: 'warning',
      message: `Plugin matcher "${sourceMatcher}" maps to TezGPT matcher "${matcher}"`,
    });
  }
  if (
