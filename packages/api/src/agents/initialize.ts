...
import type { IMongoFile, FileOwnerScope } from '@tezgpt/data-schemas';
...
function resolveProviderToolConflicts({
  ...
  if (removed > 0) {
    logger.debug(
      `[initializeAgent] Removed ${removed} ${provider} native web search tool(s); TezGPT web_search is enabled.`,
    );
  }
...
export type InitializedAgent = Agent & {
   ...
   *  registered for this agent. Authoritative TezGPT-only signal of the
...
export interface InitializeAgentParams {
  ...
  /** Explicit deployment allowlist for request types that do not carry TezGPT config on req. */
 ...
 *
 * This function is exported from @tezgpt/api and replaces the CJS version from
...
export async function initializeAgent(
  ...
  const { resendFiles, maxContextTokens, modelOptions } = extractTezGPTParams(
   ...
   * against tools TezGPT hasn't shipped yet (Claude Code's `edit_file`,
