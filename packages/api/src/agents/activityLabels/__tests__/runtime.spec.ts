...
function batchInput(overrides: Partial<PostToolBatchHookInput> = {}): PostToolBatchHookInput {
  return {
    ...
    entries: [
      {
        toolName: 'web_search',
        toolInput: { query: 'tezgpt' },
        toolUseId: 'tool-1',
        status: 'success',
        toolOutput: 'ten results about tezgpt',
      },
    ],
    ...overrides,
...
describe('createActivityLabelHook', () => {
  ...
  beforeEach(() => {
    ...
    mockInvoke.mockResolvedValue({ content: ' Searched the web for TezGPT docs. ' });
  });
  ...
  it('returns {} synchronously and fills the claimed slot when the model resolves', async () => {
    ...
    await flushDetached();
    expect(slots[0].filled).toEqual(['Searched the web for TezGPT docs.']);
  ...
  it('retries LLM resolution on the next batch after a transient failure', async () => {
    ...
    expect(slots[1].filled).toEqual(['Searched the web for TezGPT docs.']);
  });
