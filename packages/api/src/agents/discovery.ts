...
import { ResourceType, PermissionBits, EModelEndpoint } from 'tezgpt-data-provider';
import type { Agent, GraphEdge, TModelsConfig, TEndpointOption } from 'tezgpt-data-provider';
...
export interface DiscoverConnectedAgentsDeps {
   ...
   * can inject their test doubles via `jest.mock('@tezgpt/api')` — since
...
export async function discoverConnectedAgents(
  ...
  if (agent_ids?.length) {
     ...
     * `collectEdges` uses the `tezgpt-data-provider` variant (structural
