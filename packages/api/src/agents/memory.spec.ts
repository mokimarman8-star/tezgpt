...
import { Tools, MemoryScope } from 'tezgpt-data-provider';
...
const mockResolveHeaders = jest.fn((opts) => {
  ...
  for (const [key, value] of Object.entries(headers)) {
    ...
    resolved = resolved.replace(/\{\{TEZGPT_USER_EMAIL\}\}/g, user.email || '');
    resolved = resolved.replace(/\{\{TEZGPT_USER_ID\}\}/g, user.id || '');
    result[key] = resolved;
  }
  return result;
});
...
describe('Memory Agent Header Resolution', () => {
  ...
  it('should resolve user placeholders in custom endpoint headers', async () => {
    const llmConfig = {
      ...
      configuration: {
        defaultHeaders: {
          'X-User-Identifier': '{{TEZGPT_USER_EMAIL}}',
    ...
    const llmConfig = {
      ...
      configuration: {
        defaultHeaders: {
          'x-custom-api-key': '${CUSTOM_API_KEY}',
          'X-User-Identifier': '{{TEZGPT_USER_EMAIL}}',
          'X-Application-Identifier': 'TezGPT - Test',
        },
      },
    };
