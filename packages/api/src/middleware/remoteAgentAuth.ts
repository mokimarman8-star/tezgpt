...
import { getTenantId, logger, tenantStorage } from '@tezgpt/data-schemas';
import { SystemRoles, isRemoteOidcUrlAllowed } from 'tezgpt-data-provider';
import type { AppConfig, IUser, RoleMethods, UserMethods } from '@tezgpt/data-schemas';
import type { RequestHandler, Request, Response, NextFunction } from 'express';
import type { Algorithm, JwtPayload, VerifyOptions } from 'jsonwebtoken';
import type { TAgentsEndpoint } from 'tezgpt-data-provider';
import type { RequestInit } from 'undici';
import type { GetAppConfigOptions } from '../app/service';
import type { ServerRequest } from '~/types/http';
import type { ContextRequest } from './tenant';
import {
  getTezGPTRolesForOpenIdSync,
...
async function selectOpenIdRoleForOpenIdSync(
  ...
  const { rolePriority, fallbackRole } =
    user.tenantId && getTenantId() !== user.tenantId
      ? await tenantStorage.run({ tenantId: user.tenantId }, loadTezGPTRoles)
      : await loadTezGPTRoles();
  ...
  const handler = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (userResolution.status === 'missing') {
        logger.warn('[remoteAgentAuth] OIDC token valid but no matching TezGPT user');
        if (apiKeyEnabled) {
          await runApiKeyAuth(req, res, next, apiKeyMiddleware, getAppConfig);
          return;
        }
