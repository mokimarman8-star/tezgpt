import type { AppConfig, IUser, UserMethods } from '@tezgpt/data-schemas';
...
jest.mock('@tezgpt/data-schemas', () => {
  const actual = jest.requireActual('@tezgpt/data-schemas');
  return {
    ...actual,
    logger: {
      info: jest.fn(),
  ...
  describe('when OIDC verification succeeds', () => {
    ...
    it('falls back to apiKeyMiddleware when user is not found and apiKey is enabled', async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        expect.stringContaining('no matching TezGPT user'),
      );
    });
