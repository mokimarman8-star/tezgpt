import { logger, BASE_CONFIG_PRINCIPAL_ID } from '@tezgpt/data-schemas';
import {
...
import type { AppConfig, ConfigSection, IConfig, SystemCapability } from '@tezgpt/data-schemas';
...
const PROCESS_MCP_CONFIG_ERROR =
  'Process-backed MCP servers can only be configured in tezgpt.yaml';
    ...
    try {
      ...
      for (const section of BASE_ONLY_OVERRIDE_SECTIONS) {
        if (section in filteredOverrides) {
          ...
          logger.warn(
            `[adminConfig] Stripping base-only config section "${section}" - configure it in tezgpt.yaml instead`,
          );
        }
      }
    ...
    try {
      ...
      const validEntries = entries.filter((entry) => {
        if (isBaseOnlyFieldPath(entry.fieldPath)) {
          logger.warn(
            `[adminConfig] Stripping base-only config field "${entry.fieldPath}" - configure it in tezgpt.yaml instead`,
          );
          return false;
        }
  ...
  async function deleteConfigField(req: ServerRequest, res: Response): Promise<Response> {
    try {
      ...
      if (isBaseOnlyFieldPath(fieldPath)) {
        logger.warn(
          `[adminConfig] Ignoring delete for base-only config field "${fieldPath}" - configure it in tezgpt.yaml instead`,
        );
        return res.status(200).json({ message: 'No actionable field path provided' });
      }
