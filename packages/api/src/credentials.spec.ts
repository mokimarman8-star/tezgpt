...
const credentialRuntimeKey = Symbol.for('tezgpt.credentials.runtime');
...
describe('credentials', () => {
  ...
  beforeEach(() => {
    ...
    tempDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'tezgpt-credentials-'));
    tempFile = path.join(tempDirectory, '.env.temp');
    process.env = {
      ...originalEnv,
      TEZGPT_TEMP_CREDENTIALS_PATH: tempFile,
    };
  ...
  it('does not overwrite an explicitly selected environment file', () => {
    ...
    process.env.TEZGPT_TEMP_CREDENTIALS_PATH = environmentFile;
