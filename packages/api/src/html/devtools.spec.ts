...
describe('query devtools HTML bootstrap', () => {
  const html =
    '<!DOCTYPE html><html lang="en-US"><head><title>TezGPT</title></head><body></body></html>';
  ...
  it('injects a server-to-client flag when the debug header is present', () => {
    ...
    expect(updatedHtml).toContain('window.__TEZGPT_CONFIG__');
    expect(updatedHtml).toContain('data-tezgpt-query-devtools="true"');
