import { AnthropicEffort, ThinkingDisplay } from 'tezgpt-data-provider';
  ...
  describe('custom headers', () => {
    it('attaches admin-configured headers while preserving native Anthropic formatting', () => {
      const result = getLLMConfig('test-key', {
        ...
        headers: {
          'cf-aig-metadata': '{"user_email":"{{TEZGPT_USER_EMAIL}}","app":"tezgpt"}',
        },
      });
      ...
      expect((clientOptions?.defaultHeaders as Record<string, string>)['cf-aig-metadata']).toBe(
        '{"user_email":"{{TEZGPT_USER_EMAIL}}","app":"tezgpt"}',
      );
