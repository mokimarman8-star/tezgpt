...
import { CacheKeys } from 'tezgpt-data-provider';
...
const INSTRUMENTED_CACHE = Symbol('tezgpt.redisTelemetry.instrumentedCache');
...
export function finishRedisRequestTelemetry(telemetry: RedisRequestTelemetry): void {
  ...
  telemetry.span.setAttributes({
    'tezgpt.redis.calls': telemetry.calls,
    'tezgpt.redis.duration_ms': roundedMilliseconds(telemetry.durationMs),
    'tezgpt.redis.errors': telemetry.errors,
    'tezgpt.redis.max_call_ms': roundedMilliseconds(telemetry.maxCallMs),
    'tezgpt.redis.operations': [...telemetry.operations].sort(),
    'tezgpt.redis.use_cases': [...telemetry.useCases.keys()].sort(),
  });
  ...
  for (const [useCase, summary] of detailedUseCases) {
    const prefix = `tezgpt.redis.${useCase}`;
    telemetry.span.setAttributes({
