...
describe('redisTelemetry', () => {
  ...
  it('aggregates successful and failed operations onto the request span', async () => {
    ...
    expect(attributes).toMatchObject({
      'tezgpt.redis.calls': 2,
      'tezgpt.redis.errors': 1,
      'tezgpt.redis.operations': ['eval', 'get'],
      'tezgpt.redis.use_cases': ['auth_user_doc', 'rate_limit'],
      'tezgpt.redis.auth_user_doc.calls': 1,
      'tezgpt.redis.auth_user_doc.errors': 0,
      'tezgpt.redis.rate_limit.calls': 1,
      'tezgpt.redis.rate_limit.errors': 1,
    });
    expect(attributes['tezgpt.redis.duration_ms']).toEqual(expect.any(Number));
    expect(attributes['tezgpt.redis.max_call_ms']).toEqual(expect.any(Number));
  });
