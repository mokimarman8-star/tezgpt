...
describe('createSseStreamTelemetry', () => {
  it('records normal stream completion attributes', () => {
    ...
    expect(startSpan).toHaveBeenCalledWith(
      'tezgpt.sse.stream',
      {
        ...
        attributes: expect.objectContaining({
          ...
          'tezgpt.stream.id': 'stream-1',
          'tezgpt.stream.resume': false,
          'tezgpt.stream.route': '/api/agents/chat/stream/:streamId',
        }),
      },
      context.active(),
    );
    ...
    expect(span.setAttributes).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        'tezgpt.stream.bytes.sent': Buffer.byteLength(payload),
        'tezgpt.stream.chunks.count': 1,
        'tezgpt.stream.completed': true,
        'tezgpt.stream.end_reason': 'done',
        'tezgpt.stream.error_event_emitted': false,
        'tezgpt.stream.final_event_emitted': true,
        'tezgpt.stream.final_event_written': true,
        'tezgpt.stream.time_to_first_chunk_ms': expect.any(Number),
      }),
    );
  ...
  it('records client aborts on close before writableEnded', () => {
    ...
    expect(span.setAttributes).toHaveBeenCalledWith(
      expect.objectContaining({
        'tezgpt.stream.completed': false,
        'tezgpt.stream.end_reason': 'client_aborted',
      }),
    );
  ...
  it('preserves subscribe_failed as the terminal reason when an error event is sent', () => {
    ...
    expect(span.setAttributes).toHaveBeenCalledWith(
      expect.objectContaining({
        'tezgpt.stream.completed': false,
        'tezgpt.stream.end_reason': 'subscribe_failed',
        'tezgpt.stream.error_event_emitted': true,
      }),
    );
    expect(span.setStatus).toHaveBeenCalledWith({ code: SpanStatusCode.ERROR });
  ...
  it('does not count writes after the stream span has ended', () => {
    ...
    expect(span.setAttributes).toHaveBeenCalledWith(
      expect.objectContaining({
        'tezgpt.stream.bytes.sent': Buffer.byteLength(firstPayload),
        'tezgpt.stream.chunks.count': 1,
      }),
    );
    expect(span.end).toHaveBeenCalledTimes(1);
