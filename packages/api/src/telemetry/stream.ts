...
const STREAM_SPAN_NAME = 'tezgpt.sse.stream';
...
class SseStreamSpanTelemetry implements SseStreamTelemetry {
  ...
  constructor({ isResume, req, res, streamId }: SseStreamTelemetryOptions) {
    this.span = trace.getTracer('tezgpt.telemetry').startSpan(
      STREAM_SPAN_NAME,
      {
        ...
        attributes: {
          ...
          'tezgpt.stream.id': streamId,
          'tezgpt.stream.resume': isResume,
          'tezgpt.stream.route': STREAM_ROUTE,
        },
      },
      context.active(),
    );
  ...
  recordHeadersFlushed(): void {
    ...
    this.span.setAttribute('tezgpt.stream.headers_flushed', true);
  }
  ...
  recordWrite(payload: string, options?: { final?: boolean }): void {
    ...
    if (this.firstChunkMs === undefined) {
      ...
      this.span.setAttribute('tezgpt.stream.time_to_first_chunk_ms', this.firstChunkMs);
    }
  ...
  private end(reason: StreamEndReason): void {
    ...
    const attributes: Attributes = {
      ...
      'tezgpt.stream.bytes.sent': this.bytesSent,
      'tezgpt.stream.chunks.count': this.chunksCount,
      'tezgpt.stream.completed': reason === 'done',
      'tezgpt.stream.duration_ms': performance.now() - this.startTimeMs,
      'tezgpt.stream.end_reason': reason,
      'tezgpt.stream.error_event_emitted': this.errorEventEmitted,
      'tezgpt.stream.final_event_emitted': this.finalEventEmitted,
      'tezgpt.stream.final_event_written': this.finalEventWritten,
    };

    if (this.firstChunkMs !== undefined) {
      attributes['tezgpt.stream.time_to_first_chunk_ms'] = this.firstChunkMs;
    }
