# Dynamic Theme System for @tezgpt/client

## Versioned theme definitions

New themes should use the versioned `ThemeDefinition` interface. Definitions are data-only, may
provide separate light and dark overrides, and resolve missing values against TezGPT's bundled
...
```bash
npm install @tezgpt/client
```

### 2. Wrap Your App with ThemeProvider

```tsx
import { ThemeProvider } from '@tezgpt/client';
...
```js
const libreChatTailwindPreset = require('@tezgpt/client/tailwind-preset');
...
module.exports = {
  ...
  content: [
    ...
    './node_modules/@tezgpt/client/dist/**/*.js',
  ],
  darkMode: ['class'],
  theme: {
    extend: {
...
```tsx
import { IThemeRGB } from '@tezgpt/client';
...
```tsx
import { useTheme } from '@tezgpt/client';
...
```tsx
import { ThemeContext, ThemeProvider } from '@tezgpt/client';
```
...
```tsx
import { ThemeProvider, defaultTheme, darkTheme } from '@tezgpt/client';
...
This theme system is part of the @tezgpt/client package.
