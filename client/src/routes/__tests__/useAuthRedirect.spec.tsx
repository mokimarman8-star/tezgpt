 ...
 * with subdirectory deployments (e.g., /tezgpt)
 */
...
describe('useAuthRedirect', () => {
  ...
  it('should respect router basename when redirecting (subdirectory deployment)', async () => {
    ...
    // Test with basename="/tezgpt" (simulates subdirectory deployment)
    const router = createTestRouter('/tezgpt');
    const { getByTestId } = render(<RouterProvider router={router} />);

    // Full pathname includes basename
    expect(router.state.location.pathname).toBe('/tezgpt/');

    // Wait for the redirect - router handles basename internally
    await waitFor(
      () => {
        // Router state pathname includes the full path with basename
        expect(router.state.location.pathname).toBe('/tezgpt/login');
        expect(getByTestId('login-page')).toBeInTheDocument();
      },
      { timeout: 1000 },
    );
  ...
  it('should not include basename in redirect_to param (prevents path doubling)', async () => {
     ...
     */
    const router = createTestRouter('/tezgpt', '/tezgpt/c/abc123');
    render(<RouterProvider router={router} />);
    ...
    await waitFor(
      () => {
        ...
        /** redirect_to should be /c/abc123, NOT /tezgpt/c/abc123
         * because navigate() with basename will re-add the prefix */
        expect(redirectTo).toBe('/c/abc123');
        expect(redirectTo).not.toContain('/tezgpt/');
      },
      { timeout: 1000 },
    );
  });
