...
import { QueryKeys, LocalStorageKeys } from 'tezgpt-data-provider';
...
describe('ChatTitleInTab', () => {
  beforeEach(() => {
    localStorage.clear();
    localStorage.setItem(LocalStorageKeys.APP_TITLE, 'TezGPT');
  ...
  it('keeps the app title when enabling titles for a new chat', () => {
    localStorage.setItem(LocalStorageKeys.APP_TITLE, 'Custom TezGPT');
    document.title = 'Custom TezGPT';
    renderToggle({
      route: '/c/new',
  ...
  it('preserves page-specific titles outside chat routes', () => {
    ...
    expect(document.title).toBe('Agent Marketplace | TezGPT');
  });
});
