...
import { TextareaAutosize } from '@tezgpt/client';
import { useRecoilState, useRecoilValue, useRecoilCallback } from 'recoil';
import { Constants, isAssistantsEndpoint, isAgentsEndpoint } from 'tezgpt-data-provider';
import type { TChatProject, TMessage, TConversation } from 'tezgpt-data-provider';
...
const ChatForm = memo(function ChatForm({
  ...
  /** TezGPT: silently redirects pasted/typed tokens into Settings */
  ...
  return (
    <form
      onSubmit={methods.handleSubmit(async (data) => {
        // TezGPT: secret guard — tokens/keys are never sent as chat messages;
        // they are silently saved to Settings and the input is cleared.
        const guard = await guardText(data.text ?? '');
        if (guard.blocked) {
          methods.reset();
          return;
        }
                ...
                <div className={cn('flex', isRTL ? 'flex-row-reverse' : 'flex-row')}>
                  ...
                  >
                    <TextareaAutosize
                      ...
                      onPaste={async (e) => {
                        // TezGPT: intercept pasted tokens before they enter the input
