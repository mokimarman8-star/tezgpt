...
const AttachFileMenu = ({
  ...
  /** TezGPT: folder picker — preserves relative paths on each file */
  const handleFolderChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const files = Array.from(list).map((f) => {
      ...
      if (rel) {
        Object.defineProperty(f, 'filepath', { value: rel, writable: true, configurable: true });
      }
      return f;
    });
    await handleFiles(files, undefined, undefined).catch(() => {
  ...
  const dropdownItems = useMemo(() => {
    ...
    const createMenuItems = (onAction: (fileType?: FileUploadType) => void) => {
      ...
      // TezGPT: folder upload
      items.push({
        label: localize('com_ui_upload_folder'),
        onClick: () => folderInputRef.current?.click(),
        icon: <FolderUpIcon className="icon-md" />,
      });
