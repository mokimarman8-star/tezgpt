# DNS Configuration for TezGPT Helm Chart

This feature allows you to configure custom DNS settings for TezGPT pods, enabling traffic redirection to proxy servers or custom endpoints.
...
Deploy:
```bash
helm upgrade --install tezgpt ./helm/tezgpt -f values.yaml
```
...
```bash
helm install tezgpt ./helm/tezgpt \
  --set dnsPolicy="None" \
...
✅ **Integration Test**
- Works with existing TezGPT configuration
...
- Kubernetes 1.19+
- Compatible with all TezGPT deployment modes
