...
kind: Secret
metadata:
  name: tezgpt-credentials-env
  namespace: <tezgpt-chart-namespace>
type: Opaque
stringData:
...
Dependant of the Model you want to use, [create Credentials in your provider](https://docs.tezgpt.app/install/configuration/ai_setup.html) and add them to the Secret:
```yaml
apiVersion: v1
kind: Secret
. . . .
...
Set `tezgpt.adminPanelUrl` to the admin panel base URL used for OAuth/SSO
...
end with a trailing `/` because TezGPT appends `/auth/...` callback paths.
...
This renders `ADMIN_PANEL_URL` for TezGPT's admin OAuth flow. For OpenID SSO,
also register this TezGPT callback URL with your identity provider:

```text
https://<tezgpt-domain>/api/admin/oauth/openid/callback
```
...
After every TezGPT replica is on the v2-capable image and all active
generations owned by the old image have drained, set
`tezgpt.configEnv.GENERATION_PROTOCOL_VERSION="2"` in a second rollout.
...
`LANGFUSE_FANOUT_COLLECTOR_URL` for the TezGPT app unless those values are
already provided in `tezgpt.configEnv`.

Set `tezgpt.configEnv.LANGFUSE_FANOUT_TENANT_EXPORT_DISABLED=true` to keep
...
map rendered into TezGPT and the fanout gateway. Tenant API keys can still be added
...
and removal of TezGPT-only routing attributes before export.
