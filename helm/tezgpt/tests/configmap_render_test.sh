#!/usr/bin/env bash
# Regression test for the tezgpt ConfigMap template.
#
# Background: at one point templates/configmap.yaml ran
# `.Values.tezgpt.configYamlContent | toYaml | indent 4`. Because the
# value is already a YAML literal string, toYaml re-wrapped it in another `|`
# block scalar, producing a mounted /app/tezgpt.yaml whose first line was
# a bare `|`. js-yaml "recovered" by returning the body as a string, so
# TezGPT silently fell back to internal defaults for every config block
# (endpoints, interface, modelSpecs, ...).
#
# This test renders the ConfigMap with a sample configYamlContent and asserts
# that the rendered tezgpt.yaml is a proper YAML object preserving nested
# keys.
...
VALUES_FILE="$(mktemp -t tezgpt-configmap-values.XXXXXX)"
RENDERED_FILE="$(mktemp -t tezgpt-configmap-render.XXXXXX)"
...
helm template tezgpt "${CHART_DIR}" \
  --show-only templates/configmap.yaml \
  -f "${VALUES_FILE}" > "${RENDERED_FILE}"

# Pull the block scalar body that lives under `tezgpt.yaml: |`.
...
if [[ -z "${BODY}" ]]; then
  echo "FAIL: could not extract tezgpt.yaml body from rendered ConfigMap" >&2
  cat "${RENDERED_FILE}" >&2
  exit 1
fi
...
if [[ "${FIRST_LINE}" == "|" ]]; then
  echo "FAIL: rendered tezgpt.yaml starts with a bare '|' — configYamlContent is double-wrapped" >&2
...
if ! printf '%s\n' "${BODY}" | grep -qE '^version: 1\.3\.11$'; then
  echo "FAIL: expected top-level 'version: 1.3.11' in rendered tezgpt.yaml" >&2
  printf '%s\n' "${BODY}" >&2
  exit 1
fi

if ! printf '%s\n' "${BODY}" | grep -qE '^endpoints:$'; then
  echo "FAIL: expected top-level 'endpoints:' key in rendered tezgpt.yaml" >&2
  printf '%s\n' "${BODY}" >&2
  exit 1
fi

if ! printf '%s\n' "${BODY}" | grep -qE '^  agents:$'; then
  echo "FAIL: expected nested 'endpoints.agents' key in rendered tezgpt.yaml" >&2
  printf '%s\n' "${BODY}" >&2
  exit 1
fi
...
if not isinstance(doc, dict):
    sys.stderr.write(f"FAIL: parsed tezgpt.yaml is {type(doc).__name__}, expected dict\n")
    sys.exit(1)
  ...
  node)
    BODY="${BODY}" node -e '
      ...
      if (doc === null || typeof doc !== "object" || Array.isArray(doc)) {
        console.error("FAIL: parsed tezgpt.yaml is " + typeof doc + ", expected object");
        process.exit(1);
      }
...
echo "PASS: rendered tezgpt.yaml is a proper YAML document with nested keys preserved"
