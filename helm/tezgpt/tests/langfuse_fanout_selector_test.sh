...
#
# The fanout collector must not share the main TezGPT app selector labels.
...
RENDER_CHART_DIR="$(mktemp -d -t tezgpt-fanout-chart.XXXXXX)"
RENDERED_FILE="$(mktemp -t tezgpt-fanout-render.XXXXXX)"
INVALID_RENDER_ERROR="$(mktemp -t tezgpt-fanout-invalid-key.XXXXXX)"
COLLISION_RENDER_ERROR="$(mktemp -t tezgpt-fanout-colliding-key.XXXXXX)"
...
helm template tezgpt "${RENDER_CHART_DIR}" \
...
if helm template tezgpt "${RENDER_CHART_DIR}" \
  --set langfuseFanout.enabled=true \
  --set langfuseFanout.central.authHeaderSecret.name=langfuse-central \
  --set langfuseFanout.redis.uri=redis://langfuse-fanout-redis:6379 \
...
const mainService = find('Service', 'tezgpt-tezgpt');
const fanoutService = find('Service', 'tezgpt-tezgpt-langfuse-fanout');
const fanoutDeployment = find('Deployment', 'tezgpt-tezgpt-langfuse-fanout');
...
if (
  ...
  'http://tezgpt-tezgpt-langfuse-fanout.default.svc.cluster.local:4318'
) {
...
console.log('PASS: Langfuse fanout selectors are isolated from the main TezGPT Service');
NODE
