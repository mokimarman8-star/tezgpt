...
**Repos:** `tezgpt-ai/agents` (published as `@librechat/agents`, local path `~/agentus`), `tezgpt-ai/TezGPT`

**Verified dependency landscape (2026-07-28):**

| Dependency | State | What it means for this spec |
|---|---|---|
| Background capability core (`packages/api/src/agents/background.ts` + `background.spec.ts`) | **merged in TezGPT main** | the reference implementation exists today; clone it freely |
| [#14407](https://github.com/tezgpt-ai/TezGPT/pull/14407) — Background Execution Toggles for Actions & Plugin Tools | **open** | this is the "bg tools open PR"; only slice 5 (builder toggles) sequences after it |
| Activity labels, SDK side (`Run.generateActivityLabel`, `src/prompts/activityLabel.ts`, `src/types/activityLabel.ts`, `ACTIVITY_LABEL_PROMPT`) | **merged & published** in `@librechat/agents@3.3.0` ([tezgpt-ai/agents#327](https://github.com/tezgpt-ai/agents/pull/327)) | treat as existing API, build on it directly |
| [#14391](https://github.com/tezgpt-ai/TezGPT/pull/14391) — Activity Groups With Fast-Model Headers (TezGPT host side) | **open, near merge** | reshapes `ToolCallGroup` grouping and header precedence; §5.2, §9.2, and §10.10 assume it lands first and specify the composition explicitly |
...
Not per-tool capabilities, so out of scope for the family: `stateful_code_sessions`, `skills`, `subagents`, HITL approval, tool output references, eager event execution, activity labels (run-scoped, config-gated per endpoint via `tezgpt.yaml`, not per-tool). Those are agent- or run-scoped.
...
`tezgpt-ai/agents` already defines every coding tool name as a `Constants` member and groups them into exported arrays — `src/common/enum.ts` for the local groupings, `src/types/tools.ts` for the cloudflare groupings. Do not hand-maintain a parallel list; key off these.
...
| `web_search` | `WEB_SEARCH` | SDK constant; TezGPT's implementation is host-side |

**Host (`tezgpt-ai/TezGPT`)**
...
`edit_file` exists in both surfaces: the SDK coding suite (`Constants.EDIT_FILE`) and TezGPT's skill-authoring tool (`EDIT_FILE_TOOL_NAME`, matched by `FILE_AUTHORING_TOOLS` in `useStepHandler.ts` alongside `create_file`). Consequences:
...
Everything here ships as a minor `@librechat/agents` release (3.4.x), mirroring how #327 shipped the activity-label SDK surface ahead of its host PR. TezGPT's host slice pins the bump; older SDK versions simply never emit intents, so there is no fallback path to write.

---

## 8. Implementation: TezGPT host
...
- `tezgpt.example.yaml` capability list plus a comment
...
The SDK layer is **shipped**: `Run.generateActivityLabel`, `RunActivityLabelOptions`, `ActivityLabelToolEntry`, `ACTIVITY_LABEL_PROMPT` in `src/prompts/activityLabel.ts`, and the UI-only `activity_label` content type are all published in `@librechat/agents@3.3.0` (tezgpt-ai/agents#327). The host layer that consumes it is [#14391](https://github.com/tezgpt-ai/TezGPT/pull/14391), open and near merge. This spec builds on both; §10.10 covers the UI composition. What remains here is the trace story and the anchor ledger.
...
- Label generations are traced through `run.generateActivityLabel` with the **conversation as the Langfuse session** and a per-batch trace seed, so they group under their conversation (trace name `TezGPT Activity Label`, tags `["tezgpt", "activity-label"]`) instead of appearing as orphans.
...
- **Tagging:** mirror the convention — tag intent-annotated spans `["tezgpt", "tool-intent"]` so both label layers are queryable as families in Langfuse.
...
1. **SDK core** (`tezgpt-ai/agents`). `intentArg.ts`, `applyOutcome`, `outcome` / `outcome_patch` on results, `ToolNode` threading, unit tests. No behavior change until a tool opts in.
2. **SDK native tools** (`tezgpt-ai/agents`). Schema-level injection per §6.2, coverage pin test, all three engines. Slices 1–2 can ship as one minor release (3.4.x), mirroring how #327 shipped ahead of its host PR.
3. **Host** (TezGPT, requires the published SDK bump). `intent.ts`, `handlers.ts` strip, capability plumbing through all four routes, schema and config surface, `web_search`, `create_file` / `edit_file`, memory tools, tests cloned from `background.spec.ts`.
4. **UI** (TezGPT, **rebases on #14391's `ToolCallGroup` shape — land after it**). Partial-JSON parser, per-card label, three-phase group header, shimmer, sanitization, i18n fallbacks.
5. **Builder toggles** (TezGPT, **after #14407** so the switches extend its shared component rather than forking it). Including encoded-domain aliasing and the bulk toggle.
...
**Documentation** in `tezgpt.app`: a single **"Tool capability conventions"** page documenting `defer_loading`, `allowed_callers`, `run_in_background`, and `describe_intent` as one family, with the shared shape spelled out (per-tool `tool_options` key, admin capability gate, injection or metadata mechanism, self-spawn stripping obligation, token cost). Plus a `tezgpt.example.yaml` entry, and a cross-reference from the activity-labels docs explaining the two-layer labeling model (§11.2) so admins understand they compose rather than compete. This page is the deliverable that turns four features into a convention, so it is not optional.
