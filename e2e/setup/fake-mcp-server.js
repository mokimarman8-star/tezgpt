...
const APPROVAL_AUDIT_DIR = path.join('/tmp', 'tezgpt-e2e-approval-audit');
...
server.registerTool(
  'remember_fact',
  {
    description: 'Stores a deterministic fact for TezGPT mock end-to-end tests.',
    inputSchema: {
      fact: z.string().optional(),
    },
  },
  async ({ fact = 'TezGPT MCP e2e fact' }) => ({
    content: [
      {
        type: 'text',
...
server.registerTool(
  'recall_fact',
  {
    description: 'Returns a deterministic fact for TezGPT mock end-to-end tests.',
    inputSchema: {},
  },
  async () => ({
    content: [
      {
        type: 'text',
        text: 'E2E MCP memory recalled: TezGPT can persist MCP tools on agents.',
      },
    ],
  }),
);
...
server.registerTool(
  ...
  {
    description:
      'Echoes reviewed input so TezGPT mock end-to-end tests can verify tool approval decisions.',
    inputSchema: {
      value: z.string(),
