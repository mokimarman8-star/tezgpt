...
function startReplica(port, index, mongoUri) {
  ...
  child.once('exit', (code, signal) => {
    if (!shuttingDown) {
      console.error(
        `[e2e] TezGPT replica ${index} exited unexpectedly (${signal || `code ${code}`})`,
      );
...
async function waitForReplica(port) {
  ...
  throw new Error(`[e2e] TezGPT replica on port ${port} did not become ready`);
}
...
async function startCluster() {
  ...
  const mongoUri = new URL('TezGPT-e2e', mongoServer.getUri()).toString();
...
startCluster().catch((error) => {
  console.error('[e2e] Failed to start TezGPT replicas:', error);
  void shutdown(1);
});
