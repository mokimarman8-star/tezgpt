...
function registerBackendAlias() {
  ...
  require('module-alias')({
    base: path.dirname(require.resolve('@tezgpt/backend/package.json')),
  });
}
...
export default async function cleanupUser(user: TUser) {
  ...
  const { connectDb } = require('@tezgpt/backend/db/connect');
  const {
    findUser,
    deleteConvos,
    deleteMessages,
    deleteAllUserSessions,
  } = require('@tezgpt/backend/models');
  const {
    User,
    Balance,
    Transaction,
    ...
    Token,
    Group,
  } = require('@tezgpt/backend/db/models');
