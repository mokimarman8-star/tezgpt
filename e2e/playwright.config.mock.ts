...
/** Must match the `e2e-http` server URL in e2e/config/tezgpt.e2e.yaml. */
...
const configTemplatePath = path.resolve(rootPath, 'e2e/config/tezgpt.e2e.yaml');
const configPath = path.resolve(rootPath, 'e2e/.generated/tezgpt.e2e.yaml');
...
const baseEnv = {
  ...
  /** Loaded in-process by `@tezgpt/api`'s `createRun` to swap in a fake model. */
  TEZGPT_TEST_RUN_HOOK: fakeModelHookPath,
 ...
 * label server; the fake model injected via `TEZGPT_TEST_RUN_HOOK` overrides
...
export default defineConfig({
  ...
  webServer: [
    ...
    {
      // Start one TezGPT process, or a two-process topology behind a test-only proxy, after the
