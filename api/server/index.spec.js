...
describe('Server Configuration', () => {
  ...
  beforeAll(() => {
    fs.readFileSync = function (filepath, options) {
      if (filepath.includes('index.html')) {
        return '<!DOCTYPE html><html><head><title>TezGPT</title></head><body><div id="root"></div></body></html>';
      }
  ...
  beforeAll(async () => {
    ...
    fs.writeFileSync(
      ...
      '<!DOCTYPE html><html><head><title>TezGPT</title></head><body><div id="root"></div></body></html>',
    );
  ...
  it('should gate React Query Devtools config in SPA HTML by debug header', async () => {
    ...
    const debugResponse = await request(app)
      ...
      .set('x-tezgpt-enable-query-devtools', '1');
    const directIndexResponse = await request(app)
      .get('/index.html')
      .set('x-tezgpt-enable-query-devtools', '1');

    expect(defaultResponse.status).toBe(200);
    expect(defaultResponse.headers.vary).toContain('x-tezgpt-enable-query-devtools');
    expect(defaultResponse.text).not.toContain('enableQueryDevtools');

    expect(debugResponse.status).toBe(200);
    expect(debugResponse.headers.vary).toContain('x-tezgpt-enable-query-devtools');
    expect(debugResponse.text).toContain('window.__TEZGPT_CONFIG__');
    expect(debugResponse.text).toContain('data-tezgpt-query-devtools="true"');
    expect(debugResponse.text).toContain('"enableQueryDevtools":true');

    expect(directIndexResponse.status).toBe(200);
    expect(directIndexResponse.headers.vary).toContain('x-tezgpt-enable-query-devtools');
    expect(directIndexResponse.text).toContain('window.__TEZGPT_CONFIG__');
    expect(directIndexResponse.text).toContain('data-tezgpt-query-devtools="true"');
