 ...
 * @param {import('@tezgpt/api').InputItem[]} input
 ...
 * @param {import('@tezgpt/api').Response} response
 * @param {string} agentId
 * @returns {Promise<void>}
 */
 ...
 *
 * @param {import('@tezgpt/api').ResponsesRunEnvelope} envelope
...
const executeResponse = async (envelope, { req, res }) => {
  ...
  try {
     ...
     *   requestScopedConnections?: import('@tezgpt/api').RequestScopedMCPConnectionStore,
     ...
     * Responses API uses its own response-builder shape, so TezGPT-
    ...
    if (actuallyStreaming) {
      ...
      /** @type {Promise<import('tezgpt-data-provider').TAttachment | null>[]} */
      const artifactPromises = [];
      // Use Responses API-specific callback that emits tezgpt:attachment events
 ...
 * The response ID maps to a conversationId in TezGPT's storage.
 *
 * @param {import('express').Request} req
 * @param {import('express').Response} res
 */
