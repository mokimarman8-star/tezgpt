...
const { Permissions, PermissionTypes, PermissionBits } = require('tezgpt-data-provider');
...
const GENERATION_PROTOCOL_HEADER = 'x-tezgpt-generation-protocol';

/**
 * The guard ladder itself (validation, file sanitization, ownership, enqueue
 * codes) is typed logic in `@tezgpt/api` and is covered against the REAL
...
describe('SteerController (wrapper)', () => {
  ...
  it('passes an exact body+header v2 marker through the server rollout gate', async () => {
    ...
    const res = await request(buildApp())
      .post('/chat/steer')
      .set('X-TezGPT-Generation-Protocol', '2')
  ...
  it.each([
    ...
    const res = await request(buildApp())
      .post('/chat/steer')
      .set('X-TezGPT-Generation-Protocol', header)
      .send({ conversationId: 'c1', text: 'hello', generationProtocolVersion: 2 });
