...
jest.mock('@tezgpt/api', () => ({
  ...
  resolveConversationAnchor: jest.requireActual('@tezgpt/api').resolveConversationAnchor,
...
const { ErrorTypes } = require('tezgpt-data-provider');
...
describe('ResumableAgentController resume metadata', () => {
  ...
  it('persists and exactly echoes protocol v2 on a newly created generation', async () => {
    ...
    expect(res.set).toHaveBeenCalledWith('x-tezgpt-generation-protocol', '2');
    expect(res.json).toHaveBeenCalledWith({
      streamId: 'conversation-123',
      conversationId: 'conversation-123',
  ...
  it('caps a reacquired v2 lease to the immutable v1 live-job protocol', async () => {
    ...
    const req = {
      user: { id: 'user-123' },
      headers: { 'x-tezgpt-generation-protocol': '2' },
      body: {
  ...
  describe('preempt-incomplete title gating', () => {
    ...
    const { resolveTitleTiming } = require('@tezgpt/api');
