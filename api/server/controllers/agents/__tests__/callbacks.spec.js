const { Tools } = require('tezgpt-data-provider');
...
jest.mock('@tezgpt/api', () => ({
  ...
  HOST_FILE_AUTHORING_ARTIFACT_KEY: '__tezgpt_file_authoring',
...
describe('resumable event generation fencing', () => {
  ...
  it('forwards the originating job epoch with run-step events', async () => {
    const { GenerationJobManager } = require('@tezgpt/api');
...
describe('createToolEndCallback', () => {
  ...
  beforeEach(() => {
    ...
    logger = require('@tezgpt/data-schemas').logger;
    ...
    function makeCodeExecutionEvent({
      ...
      return {
        output: {
          ...
          artifact: {
            ...(hostFileAuthoring ? { __tezgpt_file_authoring: true } : {}),
            session_id: 'sess-1',
