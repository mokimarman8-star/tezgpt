 ...
 * @param {Map<string, import('@tezgpt/api').ToolInputValidationError>} [options.toolInputValidationErrors]
 ...
 * implementation (`@tezgpt/api` `createBackgroundCodeResultHandler`).
 *
 * @param {Object} params
 * @param {ServerRequest} params.req
 * @param {(params: {
...
/**
 * Helper to write attachment events in Open Responses format (tezgpt:attachment)
 * @param {ServerResponse} res - The server response object
 ...
 * Emits attachments as `tezgpt:attachment` events per the Open Responses extension spec.
 *
 * @param {Object} params
 * @param {ServerRequest} params.req
 * @param {ServerResponse} params.res
    ...
    for (const file of output.artifact.files) {
      ...
      artifactPromises.push(
        (async () => {
           ...
           * and emit a follow-up `tezgpt:attachment` with the same
