...
const mockBuildAgentScopedContext = jest.fn((...args) =>
  jest.requireActual('@tezgpt/api').buildAgentScopedContext(...args),
);
...
const { Constants, ContentTypes, EModelEndpoint } = require('tezgpt-data-provider');
const { GenerationJobManager, createStreamServices } = require('@tezgpt/api');
...
describe('AgentClient - titleConvo', () => {
          ...
          config: {
            endpoints: {
              [EModelEndpoint.anthropic]: {
                headers: { 'X-Conversation-Id': '{{TEZGPT_BODY_CONVERSATIONID}}' },
              },
            },
          },
        };
  ...
  describe('buildMessages with request and agent-scoped context attachments', () => {
    ...
    beforeEach(() => {
      jest.clearAllMocks();
      mockFormatInstructions.mockResolvedValue('');
      require('@tezgpt/api').countFormattedMessageTokens.mockImplementation(() => 42);
    ...
    it('persists canonical token counts while counting request file context for the prompt', async () => {
      const { countFormattedMessageTokens } = require('@tezgpt/api');
      const currentFile = makeTextFile('current-file', 'current.txt', 'Current turn file body');
  ...
  describe('runMemory method', () => {
    ...
    it('should handle mixed content types correctly', async () => {
      ...
      const { ContentTypes } = require('tezgpt-data-provider');
  ...
  describe('useMemory method - prelimAgent assignment', () => {
    ...
    beforeEach(() => {
      ...
      mockCheckAccess = require('@tezgpt/api').checkAccess;
      mockLoadAgent = require('@tezgpt/api').loadAgent;
      mockInitializeAgent = require('@tezgpt/api').initializeAgent;
      mockCreateMemoryProcessor = require('@tezgpt/api').createMemoryProcessor;
