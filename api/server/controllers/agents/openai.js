 ...
 *
 * @param {import('@tezgpt/api').ChatCompletionRunEnvelope} envelope
...
const executeOpenAIChatCompletion = async (envelope, { req, res }) => {
  ...
  /** @type {import('@tezgpt/api').OpenAIResponseContext} — key must be `requestId` to match the type used by createChunk/buildNonStreamingResponse */
  const context = {
    created,
  ...
  try {
     ...
     * TezGPT-style card SSE events don't apply here; only the
