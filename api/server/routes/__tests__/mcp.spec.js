...
const { getBasePath, PENDING_STALE_MS } = require('@tezgpt/api');
...
jest.mock('@tezgpt/api', () => {
  const actual = jest.requireActual('@tezgpt/api');
  return {
    ...actual,
    MCPOAuthHandler: {
    ...
    generateCheckAccess: jest.fn(({ permissionType, permissions }) => (req, res, next) => {
      const { PermissionTypes, Permissions } = require('tezgpt-data-provider');
      const isMCPUseCheck =
    ...
    // Error handling utilities (from @tezgpt/api mcp/errors)
...
const mockOAuthCompletion = (tokens) => {
  const { MCPOAuthHandler } = require('@tezgpt/api');
...
describe('MCP Routes', () => {
  ...
  beforeEach(() => {
    ...
    const { MCPOAuthHandler } = require('@tezgpt/api');
    const { getTenantId } = require('@tezgpt/data-schemas');
  ...
  describe('GET /:serverName/oauth/callback', () => {
    const { MCPOAuthHandler, MCPTokenStorage } = require('@tezgpt/api');
    const { getLogStores } = require('~/cache');
    ...
    describe('CSRF fallback via active PENDING flow', () => {
      ...
      it('should use the merged server config to defer request-scoped post-OAuth reconnect', async () => {
        ...
        const mergedServerConfig = {
          ...
          url: 'https://override.example.com/{{TEZGPT_BODY_CONVERSATIONID}}/mcp',
          source: 'config',
        };
        getLogStores.mockReturnValue({});
        require('~/config').getFlowStateManager.mockReturnValue(mockFlowManager);
      ...
      it('should resolve and forward customUserVars so header templates are substituted on first post-callback connection', async () => {
        ...
        mockRegistryInstance.getServerConfig.mockResolvedValue({});
        mockResolveAllMcpConfigs.mockResolvedValueOnce({ 'test-server': mergedServerConfig });
        require('@tezgpt/api').getUserMCPAuthMap.mockResolvedValueOnce({
        ...
        expect(response.status).toBe(302);
        expect(require('@tezgpt/api').getUserMCPAuthMap).toHaveBeenCalledWith({
          userId: 'test-user-id',
          servers: ['test-server'],
      ...
      it('should not call getUserMCPAuthMap when the server has no customUserVars', async () => {
        ...
        mockRegistryInstance.getServerConfig.mockResolvedValue({});
        mockResolveAllMcpConfigs.mockResolvedValueOnce({ 'test-server': mergedServerConfig });
        require('@tezgpt/api').getUserMCPAuthMap.mockClear();
        ...
        expect(response.status).toBe(302);
        expect(require('@tezgpt/api').getUserMCPAuthMap).not.toHaveBeenCalled();
        expect(mockMcpManager.withUserConnectionLease).toHaveBeenCalledWith(
    ...
    it('should handle OAuth callback successfully', async () => {
      ...
      const { Constants } = require('tezgpt-data-provider');
    ...
    it('defers request-scoped post-OAuth reconnect while completing the waiting tool flow', async () => {
      ...
      mockResolveAllMcpConfigs.mockResolvedValue({
        'test-server': {
          ...
          source: 'yaml',
          headers: {
            'X-Conversation-ID': '{{TEZGPT_BODY_CONVERSATIONID}}',
          },
          requiresOAuth: true,
        },
      });
  ...
  describe('POST /:serverName/reinitialize', () => {
    ...
    it('should handle server with custom user variables', async () => {
      ...
      require('~/config').getFlowStateManager.mockReturnValue({});
      require('~/cache').getLogStores.mockReturnValue({});
      require('@tezgpt/api').getUserMCPAuthMap.mockResolvedValue({
        'mcp:test-server': {
  ...
  describe('GET /connection/status', () => {
    ...
    it('should batch user variables and pass runtime context into durable status checks', async () => {
      ...
      const mcpConfig = {
        server1: {
          url: 'https://mcp.example.com/{{TEZGPT_USER_ID}}',
      ...
      require('@tezgpt/api').getUserMCPAuthMap.mockResolvedValue(userMCPAuthMap);
      ...
      expect(require('@tezgpt/api').getUserMCPAuthMap).toHaveBeenCalledTimes(1);
  ...
  describe('GET /:serverName/oauth/callback - Tenant Context', () => {
    beforeEach(() => {
      const { getTenantId, tenantStorage } = require('@tezgpt/data-schemas');
      const { MCPOAuthHandler, MCPTokenStorage } = require('@tezgpt/api');
  ...
  describe('POST /servers', () => {
    const { MCPConnection } = require('@tezgpt/api');
