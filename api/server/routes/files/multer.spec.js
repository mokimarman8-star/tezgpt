...
describe('Multer Configuration', () => {
  ...
  describe('File Filter with Real defaultFileConfig', () => {
    it('should use real fileConfig.checkType for validation', async () => {
      // Test with actual tezgpt-data-provider functions
      const {
        fileConfig,
        imageMimeTypes,
    ...
    it('should reject unsupported file types using real config', async () => {
      ...
      const originalCheckType = require('tezgpt-data-provider').fileConfig.checkType;
      const mockCheckType = jest.fn().mockReturnValue(false);
      require('tezgpt-data-provider').fileConfig.checkType = mockCheckType;
      ...
      try {
        ...
        require('tezgpt-data-provider').fileConfig.checkType = originalCheckType;
      }
    });

    it('should infer ZIP MIME type when multipart upload omits it', (done) => {
      const { mergeFileConfig } = require('tezgpt-data-provider');
    ...
    it('should use real mergeFileConfig function', async () => {
      const { mergeFileConfig, mbToBytes } = require('tezgpt-data-provider');
