...
const { createMethods, tenantStorage } = require('@tezgpt/data-schemas');
...
jest.mock('@tezgpt/api', () => ({
  ...
  getCodeExecutionBaseUrl: jest.fn((profile) =>
    profile === 'stateful'
      ? process.env.TEZGPT_CODE_BASEURL_STATEFUL
...
describe('File Routes - Delete with Agent Access', () => {
  ...
  describe('GET /files/code/download/:session_id/:fileId', () => {
    it('routes a persisted stateful fallback through the stateful Code API', async () => {
      ...
      process.env.TEZGPT_CODE_BASEURL_STATEFUL = 'https://code-stateful.example.com/v1';
