...
const { isUnpersistedPreliminaryParent } = require('@tezgpt/api');
  ...
  describe('POST /chat/abort', () => {
    describe('Authorization', () => {
      ...
      it('should allow abort when user owns the job', async () => {
        ...
        expect(response.headers['x-tezgpt-generation-protocol']).toBe('1');
    ...
    describe('Partial Response Saving', () => {
      ...
      it('should handle saveMessage errors gracefully', async () => {
        ...
        expect(response.headers['x-tezgpt-generation-protocol']).toBe('2');
      });
    });
