const { logger, redactMessage } = require('@tezgpt/data-schemas');
 ...
 */
/** Native TezGPT tools that are not in the manifest */
 ...
 * @returns {Promise<{
 *   toolDefinitions?: import('@tezgpt/api').LCTool[];
 *   toolRegistry?: Map<string, import('@tezgpt/api').LCTool>;
 *   mcpAvailableTools?: Record<string, import('@tezgpt/api').LCAvailableTools>;
 *   userMCPAuthMap?: Record<string, Record<string, string>>;
 *   hasDeferredTools?: boolean;
 * }>}
 */
async function loadToolDefinitionsWrapper({
  ...
  /** @type {Record<string, import('@tezgpt/api').LCAvailableTools>} */
      ...
      if (!isDomainAllowed) {
        logger.warn(
          `[Actions] Domain "${action.metadata.domain}" not in allowedDomains. ` +
            `Add it to tezgpt.yaml actions.allowedDomains to enable this action.`,
        );
        continue;
      }
 ...
 * @param {Record<string, import('@tezgpt/api').LCAvailableTools>} [params.mcpAvailableTools] - Run-scoped MCP tool definitions
 * @param {import('@tezgpt/api').RequestScopedMCPConnectionStore} [params.requestScopedConnections] - Run-scoped MCP connections
...
async function loadToolsForExecution({
  ...
  if (isPTC && toolRegistry) {
    ...
    try {
      /**
       * TezGPT threads per-request Code API auth through the agents
