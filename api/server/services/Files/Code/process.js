 ...
 * event, Open Responses writes the spec-shaped `tezgpt:attachment`
 ...
 *
 * @param {import('tezgpt-data-provider').CodeEnvRef} ref - Typed pointer
...
function getImageChunkBytes() {
  const parsed = Number(process.env.TEZGPT_CODE_IMAGE_CHUNK_BYTES);
    ...
    if (result.status === 'OL') {
      throw new Error(
        `Reading "${file_path}" exceeded the sandbox stdout limit (chunk ${chunkBytes} bytes). ` +
          'Lower TEZGPT_CODE_IMAGE_CHUNK_BYTES or raise SANDBOX_OUTPUT_MAX_SIZE on the runner.',
      );
    }
    if (result.stderr && (result.stdout == null || result.stdout === '')) {
      throw new Error(String(result.stderr).trim());
