...
// Mock tezgpt-data-provider with configurable file size limit
jest.mock('tezgpt-data-provider', () => {
  const actual = jest.requireActual('tezgpt-data-provider');
  return {
    ...actual,
...
const { ErrorTypes, FileContext, ResourceType } = require('tezgpt-data-provider');
...
// Mock axios — process.js now uses createAxiosInstance() from @tezgpt/api
...
describe('Code Process', () => {
  ...
  describe('processCodeOutput', () => {
    ...
    describe('non-image file processing', () => {
      ...
      it('preserves nested directory paths in the DB record while flattening the storage key', async () => {
         ...
         * file with a nested name like "test_folder/test_file.txt", TezGPT
      ...
      it('passes a NAME_MAX-aware budget to flattenArtifactPath when composing the storage key', async () => {
        ...
        const flattenSpy = require('@tezgpt/api').flattenArtifactPath;
  ...
  describe('readSandboxFile', () => {
    ...
    describe('response handling', () => {
      ...
      it('rethrows axios transport errors after logging via logAxiosError', async () => {
        const { logAxiosError } = require('@tezgpt/api');
  ...
  describe('primeFiles reupload pushes FRESH sandbox ids (Pass-N review P2)', () => {
    ...
    it('seed receives FRESH (storage_session_id, file_id) from the reupload response', async () => {
      ...
      const result = await primeFiles({
        req: { user: { id: 'user-123', role: 'USER' } },
        tool_resources: {
          execute_code: { file_ids: ['tezgpt-file-id'], files: [] },
        },
        agentId: 'agent-id',
      });
  ...
  describe('readSandboxImage', () => {
    ...
    beforeEach(() => {
      process.env.TEZGPT_CODE_BASEURL = 'http://code.test/v1';
      delete process.env.TEZGPT_CODE_IMAGE_CHUNK_BYTES;
      mockAxios.mockReset();
    });
    ...
    it('honors TEZGPT_CODE_IMAGE_CHUNK_BYTES', async () => {
      process.env.TEZGPT_CODE_IMAGE_CHUNK_BYTES = '1024';
