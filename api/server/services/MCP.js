...
const { logger, getTenantId } = require('@tezgpt/data-schemas');
 ...
 * @returns {Promise<Record<string, import('@tezgpt/api').ParsedServerConfig>>}
 */
 ...
 * @param {import('express').Request} req
 * @returns {Promise<{ configServers: Record<string, import('@tezgpt/api').ParsedServerConfig>, serverNames: string[] }>}
 */
 ...
 * `assertResolvedRuntimeConfigAllowed` in `@tezgpt/api`, whose resolution
 ...
 * @param {import('@tezgpt/api').RequestScopedMCPConnectionStore} [params.requestScopedConnections]
 * @param {import('@tezgpt/api').ParsedServerConfig} [params.serverConfig] - Used to bypass reconnect throttling for request-scoped servers.
 ...
 * @param {string | null} [params.streamId] - The stream ID for resumable mode.
 * @param {number} [params.jobCreatedAt] - The generation epoch that owns emitted events.
 * @param {import('@tezgpt/api').ParsedServerConfig} [params.config]
 * @param {import('@tezgpt/api').RequestBody} [params.requestBody]
 * @param {import('@tezgpt/api').RequestScopedMCPConnectionStore} [params.requestScopedConnections]
 * @param {Record<string, Record<string, string>>} [params.userMCPAuthMap]
 * @returns { Promise<Array<typeof tool | { _call: (toolInput: Object | string) => unknown}>> } An object with `_call` method to execute the tool input.
 */
...
async function getMCPSetupData(userId, options = {}) {
  ...
  const mcpManager = getMCPManager(userId);
  /** @type {Map<string, import('@tezgpt/api').MCPConnection>} */
  ...
  try {
    ...
    // that require user context (e.g., those with {{TEZGPT_USER_ID}} placeholders).
 ...
 * @param {string} userId - The user ID
 * @param {string} serverName - The server name
 * @param {import('@tezgpt/api').ParsedServerConfig} config - The server configuration
 * @param {Map<string, import('@tezgpt/api').MCPConnection>} appConnections - App-level connections
 * @param {Map<string, import('@tezgpt/api').MCPConnection>} userConnections - User-level connections
