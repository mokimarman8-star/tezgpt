...
const { ErrorTypes, SystemRoles, errorsToString } = require('tezgpt-data-provider');
const {
  math,
  isEnabled,
  checkEmailConfig,
...
const sendVerificationEmail = async (user) => {
  ...
  await sendEmail({
    ...
    payload: {
      appName: process.env.APP_TITLE || 'TezGPT',
 ...
 * @returns {import('@tezgpt/api').CloudFrontCookieScope | null}
 */
 ...
 * @param {import('@tezgpt/api').CloudFrontCookieScope & { orgId?: string }} [options={}]
 * @returns {boolean}
 */
...
const setAuthTokens = async (userId, res, _session = null, req = null) => {
  try {
    ...
    res.cookie('token_provider', 'tezgpt', {
      expires: new Date(refreshTokenExpires),
      httpOnly: true,
      secure: shouldUseSecureCookie(),
      sameSite: 'strict',
