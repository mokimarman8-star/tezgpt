...
// Mock @tezgpt/api — the real tenantContextMiddleware is TS and cannot be
...
jest.mock('@tezgpt/api', () => {
  ...
  const actualApi = jest.requireActual('@tezgpt/api');
...
const { getTenantId, getUserId, logger } = require('@tezgpt/data-schemas');
const {
  isEnabled,
...
describe('requireJwtAuth tenant context chaining', () => {
  ...
  it('uses OpenID JWT before TezGPT JWT when the OpenID cookie is present', async () => {
    isEnabled.mockReturnValue(true);
    mockRegisteredStrategies.add('openidJwt');
    const req = mockReq(undefined, {
...
describe('requireRumProxyAuth', () => {
  ...
  it('authenticates telemetry with the TezGPT JWT strategy without tenant or cookie refresh middleware', () => {
  ...
  it('falls back to TezGPT JWT when OpenID JWT telemetry auth fails', () => {
    isEnabled.mockReturnValue(true);
    mockRegisteredStrategies.add('openidJwt');
    const req = mockReq(undefined, {
      headers: { cookie: `token_provider=openid; openid_user_id=${signedOpenIdUserCookie()}` },
