...
const { isEnabled } = require('@tezgpt/api');
...
describe('validateImageRequest middleware', () => {
  ...
  describe('Standard TezGPT token flow', () => {
    beforeEach(() => {
      validateImageRequest = createValidateImageRequest(true);
    });
  ...
  describe('basePath functionality', () => {
    ...
    test('should validate image paths with base path', async () => {
      process.env.DOMAIN_CLIENT = 'http://localhost:3080/tezgpt';
      const validToken = jwt.sign(
        { id: validObjectId, exp: Math.floor(Date.now() / 1000) + 3600 },
        process.env.JWT_REFRESH_SECRET,
      );
      req.headers.cookie = `refreshToken=${validToken}`;
      req.originalUrl = `/tezgpt/images/${validObjectId}/test.jpg`;
    ...
    test('should validate agent avatar paths with base path', async () => {
      ...
      req.headers.cookie = `refreshToken=${validToken}`;
      req.originalUrl = `/tezgpt/images/${validObjectId}/agent-avatar.png`;
    ...
    test('should handle nested subdirectories in base path', async () => {
      process.env.DOMAIN_CLIENT = 'http://localhost:3080/apps/tezgpt';
      const validToken = jwt.sign(
        { id: validObjectId, exp: Math.floor(Date.now() / 1000) + 3600 },
        process.env.JWT_REFRESH_SECRET,
      );
      req.headers.cookie = `refreshToken=${validToken}`;
      req.originalUrl = `/apps/tezgpt/images/${validObjectId}/test.jpg`;
    ...
    test('should prevent path traversal with base path', async () => {
      ...
      req.headers.cookie = `refreshToken=${validToken}`;
      req.originalUrl = `/tezgpt/images/${validObjectId}/../../../etc/passwd`;
    ...
    test('should handle URLs with query parameters and base path', async () => {
      ...
      req.headers.cookie = `refreshToken=${validToken}`;
      req.originalUrl = `/tezgpt/images/${validObjectId}/test.jpg?version=1`;
    ...
    test('should handle URLs with fragments and base path', async () => {
      ...
      req.headers.cookie = `refreshToken=${validToken}`;
      req.originalUrl = `/tezgpt/images/${validObjectId}/test.jpg#section`;
    ...
    test('should handle HTTPS URLs with base path', async () => {
      process.env.DOMAIN_CLIENT = 'https://example.com/tezgpt';
      const validToken = jwt.sign(
        { id: validObjectId, exp: Math.floor(Date.now() / 1000) + 3600 },
        process.env.JWT_REFRESH_SECRET,
      );
