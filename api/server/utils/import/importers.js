...
} = require('@tezgpt/data-schemas');
const { EModelEndpoint, Constants, Tools, openAISettings } = require('tezgpt-data-provider');
...
function getImporter(jsonData) {
  ...
  if (jsonData.conversationId && (jsonData.messagesTree || jsonData.messages)) {
    logger.info('Importing TezGPT conversation');
    return importTezGPTConvo;
  }
...
/**
 * Imports a TezGPT conversation from JSON.
 *
 ...
 * @returns {Promise<void>} - A promise that resolves when the import is complete.
 */
async function importTezGPTConvo(
  jsonData,
  requestUserId,
  builderFactory = createImportBatchBuilder,
  userRole,
  ...
  try {
    ...
    if (jsonData.recursive) {
      ...
      throw new Error('Invalid TezGPT file format');
    }
    ...
    logger.error(`user: ${requestUserId} | Error creating conversation from TezGPT file`, error);
    throw error;
  }
}
