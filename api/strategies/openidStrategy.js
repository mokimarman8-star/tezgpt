...
const { hashToken, logger, tenantStorage } = require('@tezgpt/data-schemas');
const { Strategy: OpenIDStrategy } = require('openid-client/passport');
const { CacheKeys, ErrorTypes, SystemRoles } = require('tezgpt-data-provider');
const {
  isEnabled,
  logHeaders,
  safeStringify,
...
async function applyOpenIdRoleSync({
  ...
  const { rolePriority, fallbackRole } = user?.tenantId
    ? await tenantStorage.run({ tenantId: user.tenantId }, async () =>
        getTezGPTRolesForOpenIdSync(libreChatRoles),
      )
    : await getTezGPTRolesForOpenIdSync(libreChatRoles);
