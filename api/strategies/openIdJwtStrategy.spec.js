const { SystemRoles } = require('tezgpt-data-provider');
...
jest.mock('@tezgpt/api', () => ({
  ...
  getOpenIdEmail: jest.requireActual('@tezgpt/api').getOpenIdEmail,
  getOpenIdIssuer: jest.fn(() => 'https://issuer.example.com'),
  normalizeOpenIdIssuer: jest.requireActual('@tezgpt/api').normalizeOpenIdIssuer,
...
describe('openIdJwtStrategy – token validation', () => {
  ...
  it('requires OpenID JWTs to match the configured client audience and issuer', () => {
    withEnv({ OPENID_CLIENT_ID: 'tezgpt-client-id', OPENID_AUDIENCE: undefined }, () => {
      openIdJwtLogin(mockOpenIdConfig);
    });

    expect(capturedStrategyOptions).toMatchObject({
      audience: 'tezgpt-client-id',
  ...
  it('also accepts OPENID_AUDIENCE for providers that mint resource-bound JWTs', () => {
    withEnv({ OPENID_CLIENT_ID: 'tezgpt-client-id', OPENID_AUDIENCE: 'api://tezgpt' }, () => {
      openIdJwtLogin(mockOpenIdConfig);
    });

    expect(capturedStrategyOptions).toMatchObject({
      audience: ['tezgpt-client-id', 'api://tezgpt'],
    });
  });

  it('uses a single OPENID_AUDIENCE value when no client ID is configured', () => {
    withEnv({ OPENID_CLIENT_ID: undefined, OPENID_AUDIENCE: 'tezgpt' }, () => {
      openIdJwtLogin(mockOpenIdConfig);
    });

    expect(capturedStrategyOptions.audience).toBe('tezgpt');
  });

  it('splits comma-separated OPENID_AUDIENCE values into multiple accepted audiences', () => {
    withEnv({ OPENID_CLIENT_ID: undefined, OPENID_AUDIENCE: 'tezgpt,control-plane-web' }, () => {
      openIdJwtLogin(mockOpenIdConfig);
    });

    expect(capturedStrategyOptions.audience).toEqual(['tezgpt', 'control-plane-web']);
  });

  it('trims whitespace around comma-separated OPENID_AUDIENCE values', () => {
    withEnv(
      { OPENID_CLIENT_ID: undefined, OPENID_AUDIENCE: ' tezgpt , control-plane-web ' },
      () => {
        openIdJwtLogin(mockOpenIdConfig);
      },
    );
  ...
  it('combines OPENID_CLIENT_ID with comma-separated OPENID_AUDIENCE values and deduplicates', () => {
    withEnv(
      { OPENID_CLIENT_ID: 'tezgpt', OPENID_AUDIENCE: 'tezgpt,control-plane-web' },
      () => {
        openIdJwtLogin(mockOpenIdConfig);
      },
    );
...
describe('openIdJwtStrategy – OPENID_EMAIL_CLAIM', () => {
  ...
  beforeEach(() => {
    ...
    const realFindOpenIDUser = jest.requireActual('@tezgpt/api').findOpenIDUser;
