...
const { ErrorTypes, FileSources } = require('tezgpt-data-provider');
...
jest.mock('@tezgpt/api', () => {
  ...
  return {
    ...
    getAvatarFileStrategy: jest.fn((config, fallbackStrategy) => {
      const { FileSources } = jest.requireActual('tezgpt-data-provider');
...
describe('setupOpenId', () => {
  ...
  beforeEach(async () => {
    ...
    jest.clearAllMocks();
    isEnabled.mockImplementation(jest.requireActual('@tezgpt/api').isEnabled);
  ...
  describe('authorizationRequestParams', () => {
    ...
    it('uses the first non-empty audience when OPENID_AUDIENCE accepts multiple JWT audiences', () => {
      process.env.OPENID_AUDIENCE = ' tezgpt , control-plane-web ';
  ...
  describe('OpenID role sync', () => {
    ...
    it('wraps role lookup in tenant context for tenant users', async () => {
      ...
      const { tenantStorage } = require('@tezgpt/data-schemas');
    ...
    it('re-enforces tenant login policy after role sync changes the role', async () => {
      ...
      const { isEmailDomainAllowed } = require('@tezgpt/api');
