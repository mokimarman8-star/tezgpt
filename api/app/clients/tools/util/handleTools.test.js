...
jest.mock('~/server/services/MCP', () => ({
   ...
   *  failure. The pure sensitivity predicate is the REAL @tezgpt/api one. */
...
const { Tools, Constants } = require('tezgpt-data-provider');
const { ASK_USER_QUESTION_TOOL_NAME } = require('@tezgpt/api');
...
describe('Tool Handlers', () => {
  ...
  describe('loadTools', () => {
    ...
    it('resolves an MCP tool whose raw name itself contains the delimiter substring', async () => {
      // Regression test for https://github.com/tezgpt-ai/TezGPT/issues/14440:
      ...
      // TezGPT appends its own server suffix, the combined key has the
