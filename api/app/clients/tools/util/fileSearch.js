...
const { generateShortLivedToken, logAxiosError } = require('@tezgpt/api');
const { Tools, EToolResources } = require('tezgpt-data-provider');
...
const createFileSearchTool = async ({ userId, files, entity_id, fileCitations = false }) => {
  return tool(
    async ({ query }) => {
      ...
      /**
       * @param {import('tezgpt-data-provider').TFile & { fromAgent?: boolean }} file
