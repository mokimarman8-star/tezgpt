...
const DEPS_HASH_MARKER = path.join(ROOT_DIR, 'node_modules', '.tezgpt-deps-hash');
...
function runTurboBuild() {
  ...
  if (flags.skipClient) {
    args.push('--filter=!@tezgpt/frontend');
  }
  if (flags.force) {
    args.push('--force');
  }
