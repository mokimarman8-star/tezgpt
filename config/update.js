...
(async () => {
  ...
  if (docker) {
    ...
    console.purple('Pruning all TezGPT Docker images...');

    const imageName = singleCompose ? 'tezgpt_single' : 'tezgpt';
    try {
      execSync(`${sudo}docker rmi ${imageName}:latest`, { stdio: 'inherit' });
    } catch (_error) {
      console.purple('Failed to remove Docker image tezgpt:latest. It might not exist.');
    }
    console.purple('Removing all unused dangling Docker images...');
    execSync(`${sudo}docker image prune -f`, { stdio: 'inherit' });
    console.purple('Building new TezGPT image...');
  ...
  console.green('Your TezGPT app is now up to date! Start the app with the following command:');
  console.purple(startCommand);
  console.orange(
    "Note: it's also recommended to clear your browser cookies and localStorage for TezGPT to assure a fully clean installation.",
  );
