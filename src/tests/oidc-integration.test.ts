...
describe('OpenID Connect Federated Provider Token Integration', () => {
  ...
  describe('processOpenIDPlaceholders', () => {
    ...
    it('should replace OpenID Connect token placeholders', () => {
      const template = 'Bearer {{TEZGPT_OPENID_TOKEN}}';
    ...
    it('should replace specific OpenID Connect placeholders', () => {
      const template = `
        Access: {{TEZGPT_OPENID_ACCESS_TOKEN}}
        ID: {{TEZGPT_OPENID_ID_TOKEN}}
        User: {{TEZGPT_OPENID_USER_ID}}
        Email: {{TEZGPT_OPENID_USER_EMAIL}}
        Name: {{TEZGPT_OPENID_USER_NAME}}
    ...
    it('should handle missing token fields gracefully', () => {
      ...
      const template = 'Token: {{TEZGPT_OPENID_TOKEN}}, Email: {{TEZGPT_OPENID_USER_EMAIL}}';
  ...
  describe('Integration with resolveHeaders', () => {
    it('should resolve OpenID Connect placeholders in headers for Cognito', () => {
      const headers = {
        'Authorization': '{{TEZGPT_OPENID_TOKEN}}',
        'X-User-ID': '{{TEZGPT_OPENID_USER_ID}}',
        'X-User-Email': '{{TEZGPT_OPENID_USER_EMAIL}}',
      };
    ...
    it('should work with specific access token placeholder', () => {
      const headers = {
        ...
        'X-Cognito-ID-Token': '{{TEZGPT_OPENID_ID_TOKEN}}',
      };
  ...
  describe('Integration with processMCPEnv', () => {
    it('should process OpenID Connect placeholders in MCP environment variables for Cognito', () => {
      const mcpOptions = {
        ...
        args: ['server.js'],
        env: {
          'COGNITO_ACCESS_TOKEN': '{{TEZGPT_OPENID_TOKEN}}',
          'USER_ID': '{{TEZGPT_OPENID_USER_ID}}',
          'USER_EMAIL': '{{TEZGPT_OPENID_USER_EMAIL}}',
        },
      };
    ...
    it('should process OpenID Connect placeholders in MCP headers for HTTP transport', () => {
      const mcpOptions = {
        ...
        headers: {
          'Authorization': 'Bearer {{TEZGPT_OPENID_ACCESS_TOKEN}}',
          'X-Cognito-User-Info': '{{TEZGPT_OPENID_USER_EMAIL}}',
          'X-Cognito-ID-Token': '{{TEZGPT_OPENID_ID_TOKEN}}',
        },
      };
    ...
    it('should handle AWS-specific MCP server configuration', () => {
      const awsMcpOptions = {
        ...
        env: {
          'AWS_COGNITO_TOKEN': '{{TEZGPT_OPENID_ACCESS_TOKEN}}',
          'AWS_COGNITO_ID_TOKEN': '{{TEZGPT_OPENID_ID_TOKEN}}',
          'COGNITO_USER_SUB': '{{TEZGPT_OPENID_USER_ID}}',
        },
      };
  ...
  describe('Security and Edge Cases', () => {
    it('should not process OpenID Connect placeholders for expired tokens', () => {
      ...
      expect(resolvedHeaders['Authorization']).toBe('Bearer {{TEZGPT_OPENID_TOKEN}}');
    });
    ...
    it('should handle multiple placeholder instances in same string', () => {
      const template = '{{TEZGPT_OPENID_TOKEN}}-{{TEZGPT_OPENID_TOKEN}}-{{TEZGPT_OPENID_USER_ID}}';
